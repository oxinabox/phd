...
facts("break and move") do
	...
	@fact Training.break_and_move!(zero_start,  sdict([[10f0, 0]]), 0.) -->	[[10f0,0]]
	@fact Training.break_and_move!(zero_start,  sdict([[-10f0, 0]]), 0.) --> [[0f0,0]]
	@fact Training.break_and_move!(zero_start,  sdict([[5f0, 0],[-5,0]]), 0.) --> [[5f0,0],[-5f0,0]]


	srand(10) #No actual random chance, in tests
	forces   = [ 1.+round(10.0*rand(Float32, 2)) for ii in 1:1000]
	@fact Training.break_and_move!(zero_start, sdict(forces),0.0) |> length  -->  greater_than(0) "should produce some forces after"


	@fact Training.break_and_move!([[1f0,0],[2f0,0]],
								sdict([[50f0,-3],[-10f0,6]],2), 0.0) |> Set --> Set( [[1f0,0],[52f0,-3],[-8f0,6]]) "Don't loose the other word senses"
