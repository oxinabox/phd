   	...
   	\block{Machine learning methods for determining POV}{
   		\innerblock{Classical Features + Logistic Regression}{
   			\imp{Features}: position, and occurrence frequency,
   			 plus parts of speech cooccurring frequency. Total 200 dims.\\
   		...
   		\innerblock{Word Embeddings + RBF-SVM}{
   			\imp{Features}: concatentation of FastText word embeddings for the adjacent words, averaged over all occurrences.
   			Total 600 dims.
   			\\
   			\imp{Scoring}: use RBF-SVM model on if POV or not, $S_i=\frac{P(f_i)}{\sum_{\forall j} P(f_j)}$\\
   			\imp{Result}: \imp{\color{green}generally great}. However, due to high dimensionality needs sufficient training data from other labelled books.
   		}
   	}
