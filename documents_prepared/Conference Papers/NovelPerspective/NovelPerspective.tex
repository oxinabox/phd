...
\begin{figure*}
	\resizebox{\textwidth}{!}{
		...
		\node(classify)[draw, right=of features] {Main Character\\ Classification};
		...
		\path (features) edge node[above]{feature-vector\\character-name\\pairs
		} (classify);
		\path (classify) edge node[above]{character name}  (end);
		\end{tikzpicture}
	}
	\caption{\label{fig:classify}
	}
\end{figure*}
...
During training, from each chapter in the training dataset,
...
We define a ``Word Embedding Feature-Set'' using FastText word vectors
...
%		every row 3 column Acc /.style={bf content},
