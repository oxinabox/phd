...
First the raw text is enriched with part of speech and named entity tags, which features are extracted for each named entity.
...
This has an a total of 600 features.
...
This predicted probability of that feature vector being for POV character was used as the score for that character.

This may seem similar to an one-vs-rest approach to using binary classifiers for multiclass classification.
...
We  during preliminary testing found that many classifiers had similarly high degree of success, and so chose the simplest.
...
%but consider it as supplementary training data in \Cref{sec:results-extradata}.
...
This corresponds to the case that a normal user can enter multiple aliases into our application when selecting sections to keep.
...
\begin{table}
	\begin{adjustbox}{max width=\columnwidth}
		\pgfplotstabletypeset[%
			every row 1 column Acc/.style={
				postproc cell content/.style={@cell content/.add={$\bf}{$}}
			},
			every row 8 column Acc/.style={
				postproc cell content/.style={@cell content/.add={$\bf}{$}}
			},
			%
			every nth row={5}{before row=\cmidrule{2-4}},
...
Of the two machine learning models the Classical Feature set performs best for the ASOIAF test set,
and the Word Embeddings for the SOC testset.
This asymmetry can potentially be attributed to the different amounts of training data for each task.
The word embedding feature set has 600 dimensions, it seems that smaller SOC training set (used for the ASOIAF test set)
allowed it to over-fit, causing poor generalisation.
Where-as the simpler classic feature set was more robust.
The opposite is true for the other task; the larger amount of training data allowed the capacity of the word embeddings to be exploited,
resulting in higher accuracy..

We are surprised at how well the word embeddings perform.
The features do not include any information on the occurrence frequency of the named entity.
According to our examination of the logistic regression co-efficient is the most important feature for the classical feature set.
The word embeddings which provide just context word information can still produce accurate results, given enough context information,
...
with the predicted main character(/s) in the left, and an excerpt from the beginning of the section on the right.
...
A primary issues is the limited amount of labelled training data.
