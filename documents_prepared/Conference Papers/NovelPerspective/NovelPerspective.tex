...
\begin{figure*}
	...
	\caption{The general structure of the character classification systems. This is step 3 of the whole pipeline shown in \Cref{fig:fullprocess}. \label{fig:classify}}
\end{figure*}
...
\begin{table}
	\begin{adjustbox}{max width=\columnwidth}
		\pgfplotstabletypeset[%
			every row 3 column Acc/.style={
				postproc cell content/.style={@cell content/.add={$\bf}{$}}
			},
			every row 10 column Acc/.style={
				postproc cell content/.style={@cell content/.add={$\bf}{$}}
			},
			every row 14 column Acc/.style={
				postproc cell content/.style={@cell content/.add={$\bf}{$}}
			},
			%
			every nth row={6}{before row=\cmidrule{2-4}},
...
The datasets are, as was presented in \Cref{tbl:datasets}, of very different sizes.
...
The models deployed to our demonstration system (\Cref{sec:demonstration}) use the complete set of three datasets for training to achieve the best possible results.
