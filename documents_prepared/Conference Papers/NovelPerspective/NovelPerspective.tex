...
\begin{figure*}
	...
	\caption{The general structure of the character classification systems. This repeated for each section of the book during step 3 of the pipeline shown in \Cref{fig:fullprocess}. \label{fig:classify}}
\end{figure*}
...
These are both potentially useful to the end-user in our deployed system (\Cref{sec:demonstration}), and used to gauge the performance of the more complicated systems in the evaluations presented in \Cref{sec:results-and-discussion}.
...
We believe that difference between the top result for a method and the result using the combined training sets is too small to be meaningful.
...
To maximise the generalisability of the NovelPerspective prototype (see \Cref{sec:demonstration}), we deploy models trained on the combination all three datasets.

More generally the machine learning model results for all the models were very close.
The exception to this is the SVM trained on SOC, using word embedding features, which for both ASOIAF and WOT test sets performed much worse.
...
This is rough upper bound for the possible performance of these models, as imposed by the classifier and the feature set.
...
%
The result do not show a clear advantage to either machine learning feature set.
...
\Cref{tbl:restrain} shows the upper bound for this word embeddings feature set is not much higher, so without using more features, more training data would not allow an improvement.
But with more training data, and more features (wider-window), it may be that word embedding may prove better.
...
The web-app functions as was shown in \Cref{fig:fullprocess}.
