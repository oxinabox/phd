...
\resizebox{\textwidth}{!}{
	\begin{tikzpicture}[
		...
		node distance=2.4, 
		->,
		every edge/.append style={every node/.style={font=\small}},
	...
	\begin{scope}[xshift=-1cm]		
		...
		\path (start) edge[below] node[below]{original ebook \\ + settings}  (convert);
		\path (convert) edge[below] node[below]{section\\ content} (classify);
		\path (classify) edge[below] node[below]{section-character\\score list} (select);
