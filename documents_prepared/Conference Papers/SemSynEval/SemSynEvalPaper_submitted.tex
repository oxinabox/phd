...
\documentclass[onecolumn,english]{acm_proc_article-sp}
...
\category{H.3.1}{Information Storage and Retrieval}{Content Analysis and Indexing}[Abstracting methods, Linguistic processing] 
\category{I.2.7}{Artificial Intelligence}{Natural Language Processing}[Language parsing and understanding] 


\keywords{Semantic vector space representations, semantic consistency evaluation,
...
evaluation methods, such as the work of Gershamn and Tenebaum \cite{gershmanphrase}
...
The rest of the paper is organized into the following sections. \secref{Background}
...
assessnements. \secref{Methodology} describes the processes by which
the models are evaluated using our new method, and the parameters
used in the evaluation. \secref{Corpus-Construction} continues into
more details on the development of the evaluation corpora for the
semantic classification evaluation task. \secref{Results-and-Discussion}
details the results from evaluating the models and discusses the implications
for their semantic consistency. \secref{Conclusion} closes the paper
...
down-stream. The evaluation methods are not directly link to the methods
used for training. One of the more semantically focused is paraphrase
detection.
...
Sentiment Analysis is normally tasked with classifying as positive
or negative, or assigning a score, as in the Sentiment Treebank \cite{RvNTN}.
Determining the sentiment of a sentence is partially a semantic task,
but it is lacking in several areas that would be required for meaning.
For example, there is only an indirect requirement for the model to
process the subject at all. Sentiment Analysis is a key task in natural
language processing, but it is very distinct from semantic meaning.
...
``Business'' and ``Entertainment''. The topics for classification
tend to be broad. A huge spectrum of different sentences are assigned
to the same topic. It is thus too board and insufficiently specific
to evaluate the consistency of meanings. Information retrieval can
be seen as the inverse of the document classification task.
...
\begin{figure*}[t]
{\footnotesize{}\include{figs/block_overview}}{\footnotesize \par}
...
as shown in \figref{Process-Diagram}: Corpus Preparation, Model Preparation,
...
quite follow the steps shown in \figref{Process-Diagram}, as the
...
\cite{scikit-learn} was used to orchestrate the cross-validation
and to interface with the LibLinear SVM implementation \cite{LIBLIBEAR}.
...
were present in the original corpus as paraphrase pairs: $A,B$, $D,\,A$
and $B,C$ then the paraphrase group $\{A,B,C,D\}$ is found. Any
paraphrase groups containing less than 3 phrases were discarded. The
resulting sub-corpus has the breakdown as shown in \figref{msrp_corpus_hist}.
...
MSRP corpus. The grouping was carried out manually. The general guidelines
were as follows.
\begin{itemize}
...
is as in \figref{opinosis_corpus_hist}.

Further information on the construction of the corpora in this section,
and download links are available online.\footnote{{[}blinded{]}}
...
The results of performing the evaluation method described in \secref{Methodology}
are shown in \tabref{results}.
...
-- like manually to choosing the best keywords/stopwords. It loses
little performance, even though decreasing vector size by an order
...
\begin{figure*}
\includegraphics[width=1\columnwidth]{figs/msrp_agree}\hspace*{\fill}\includegraphics[width=1\columnwidth]{figs/opinosis_agree}\enspace{}
...
the similarities of the models were assessed. The heatmaps in \figref{misclass_agreement}
...
the work of Ritter et. al.\cite{RitterPosition} discussed in \subref{Evaluations-of-Semantic}
...
-- in real-world speech people trying to say the same thing do in
fact use the same words very often.
...
to word overlap. Combining the results, it can be concluded that summing
word vector representations is a practical and surprisingly effective
method for encoding the meaning of a sentence. 
