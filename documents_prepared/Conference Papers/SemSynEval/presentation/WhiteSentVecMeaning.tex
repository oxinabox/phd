...
\begin{frame}[fragile]{Paraphrase groups from Opinosis}
	\begin{itemize}
		...
		\only<3>{\item This hotel is in a good location on Fisherman's Wharf .}
...
\begin{frame}{Sum/Mean of Word Embeddings}
	\begin{itemize}
		...
		\item Abbreviated SOWE, and MOWE respectively.
	\end{itemize}
...
\begin{frame} {PV--DM} 
	\hskip-1cm
	\only<1>{\fullpagetikz{../figs/pvdm/PVDM1.pgf}}
	\only<2>{\fullpagetikz{../figs/pvdm/PVDM2.pgf}}
	\only<3>{\fullpagetikz{../figs/pvdm/PVDM3.pgf}}
\end{frame}
\endgroup
...
\begin{frame} {PV--DBOW}
	\centering
	\only<1>{\fullpagetikz{../figs/pvdbow/PVDBOW1.pgf}}
	\only<2>{\fullpagetikz{../figs/pvdbow/PVDBOW2.pgf}}
	\only<3>{\fullpagetikz{../figs/pvdbow/PVDBOW3.pgf}}
\end{frame}
...
\begin{frame}{Annex: Exact Results}
	\centering\pgfplotstabletypeset[columns={Name, msrp, opiniosis}, 
		...
		columns/Name/.style={string type}]{\resultstable}
\end{frame}
