...
\begin{tikzpicture}[
    ...
    \draw[->] (window) -- node {context words} (WSD) -- node{word sense embedding \\ (the single most likely)} (train);
    \draw[->] (window) -- node{middle word} (lookup)  -- node {sense embeddings \\ (Multiple candidates)} (WSD);
