...
%\textfloatsep 2pt plus 2pt minus 4pt
%\floatsep 2pt plus 2pt minus 4pt
\intextsep 2pt plus 2pt minus 4pt
...
\begin{table}
	\caption{Spearman rank correlation $\rho \times 100$  when evaluated on the SCWS task.}
	\begin{minipage}[t]{0.5\linewidth}
		\subcaption{For varying hyper-parameters.} \label{swscres}
	...
	\end{minipage}%	
	\begin{minipage}[t]{0.5\linewidth}
		\subcaption{Compared to other methods \label{swscEvery}.
...
We also consider the results when using a backoff to MSF when a method is unable to determine the word sense the method can report the MFS instead of returning no result (a non-attempt).
...
%				
	...
	The \mbox{\emph{\textasteriskcentered } marks} results with MSF backoff.
	} \label{samevalres}
\end{table}
