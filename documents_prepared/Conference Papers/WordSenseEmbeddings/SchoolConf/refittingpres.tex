		...
		\begin{column}{0.5\textwidth}
			\begin{block}<2->{Smoothed:}
				...
				\only<3->{
					...
					\fitmath{
					P(s_{i} \mid \c) =
					\dfrac{\sqrt[|\c|]{P(\c \mid s_{i})P(s_{i})}}
					{\sum_{s_{j}\in\s} \sqrt[|\c|]{P(\c \mid s_{j})P(s_{j})}}
					}
				}
			\end{block}			
		\end{column}		
...
%		
			\addplot[fill=darkbrown] table[y=AvgSimC, x expr=\coordindex]{\swsc}; 
			\addplot[fill=UniBlue] table[y=RefittedSim, x expr=\coordindex]{\swsc}; 
...
\begin{frame}[fragile]{Results for word sense disambiguation}
	...
	%		
	\addplot[fill=darkbrown] table[y=F1, x expr=\coordindex]{\wsd}; 
