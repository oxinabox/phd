...
%DIF ADD AligningWordSenseEmbeddings.tex   Wed Feb 15 16:59:28 2017
...
We show how our \DIFdelbegin \DIFdel{refitting based }\DIFdelend techniques improve the performance of the Adaptive Skip-Gram sense embeddings for word similarly evaluation; and how \DIFdelbegin \DIFdel{it allows }\DIFdelend \DIFaddbegin \DIFadd{they allow }\DIFaddend the embeddings to be used for lexical word sense disambiguation -- which was not possible \DIFdelbegin \DIFdel{before refitting the }\DIFdelend \DIFaddbegin \DIFadd{using the induced }\DIFaddend sense embeddings.
...
\DIFdelbegin \DIFdel{Beyond refitting to a standard lexical sense, refitting }\DIFdelend \DIFaddbegin \DIFadd{Refitting can also be used to fit }\DIFaddend to a user provided example\DIFaddbegin \DIFadd{, giving a specific sense vector for that use.
...
The rest of the paper is organised as follows: \Cref{relatedwords} \DIFaddbegin \DIFadd{briefly }\DIFaddend discusses two areas of related works\DIFdelbegin \DIFdel{, one on directly learning standard lexical sense repressions, and the other on indirectly associating induced senses with standard lexical senses}\DIFdelend . \Cref{Framework} presents our refitting method, as well as the geometric smoothing method used with it. \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\Cref{SimilarityInContext} }%DIFAUXCMD
describes the }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\Cref{Models} }%DIFAUXCMD
describes the WSI embedding models used in the evaluations. \mbox{%DIFAUXCMD
\Cref{SimilarityInContext} }%DIFAUXCMD
defines the }\DIFaddend RefittedSim measure for word similarity in context, and presents its results. \Cref{lexicalWSD} shows how the refitted sense vectors can be used for lexical \DIFdelbegin \DIFdel{word sense disambiguation}\DIFdelend \DIFaddbegin \DIFadd{WSD}\DIFaddend . Finally, the paper concludes with outlook to future works in \Cref{conclusion}.
...
Initially the sense vectors are set as the average of the single sense word embeddings \parencite{mikolov2013efficient} for the words in the WordNet gloss. \DIFdelbegin \DIFdel{Similarly, a context vector is defined as the average of all }\DIFdelend \DIFaddbegin \DIFadd{This is then also done for the }\DIFaddend words in a sentence. \DIFdelbegin \DIFdel{They then progressively label the words with the sense vector that is closest to the context vector if it is closer than a predefined }\DIFdelend \DIFaddbegin \DIFadd{The distance between the sentence average and the gloss average is used to perform WSD to label the word sense. Words are only sense labelled when the distance is within a }\DIFaddend threshold. The requirement for meeting a threshold in order to add the sense label  decreases the likelihood of training on an incorrect sense label.
...
Adding a new sense \DIFdelbegin \DIFdel{using our approach is practically instantiations}\DIFdelend \DIFaddbegin \DIFadd{is practically instantaneous}\DIFaddend , and replacing the entire sense inventory, of several hundred thousand senses, is only a matter of a few hours.
Whereas for the existing approaches \DIFdelbegin \DIFdel{adding senses would require almost fully }\DIFdelend \DIFaddbegin \DIFadd{this would require }\DIFaddend repeating the training process, \DIFdelbegin \DIFdel{often taking }\DIFdelend \DIFaddbegin \DIFadd{which will often take }\DIFaddend several days.
Refitting is a process done to \DIFdelbegin \DIFdel{sense word }\DIFdelend \DIFaddbegin \DIFadd{word sense }\DIFaddend embeddings, rather \DIFaddbegin \DIFadd{than }\DIFaddend a method for finding sense embeddings from a large corpus. 
...
\DIFdelbegin \DIFdel{Assume we are given }\DIFdelend \DIFaddbegin \DIFadd{Given }\DIFaddend a collection of induced (unlabelled) embeddings $\u={u_1,...,u_{n_u}}$, and an example sentence $\c={w_1,...,w_{n_c}}$ \DIFdelbegin \DIFdel{. We }\DIFdelend \DIFaddbegin \DIFadd{we }\DIFaddend define a function $l(\u \mid \c )$ which determines the refitted sense vector, from the unsupervised vectors and the context as:
\begin{equation} \label{eq:synth}
l(\u \mid \c ) = \sum_{\forall u_i \in \u} u_i P(u_i \mid \c)
\end{equation}

\DIFdelbegin \DIFdel{To do this, we need }\DIFdelend \DIFaddbegin \DIFadd{Bayes' Theorem can be used }\DIFaddend to estimate the posterior predictive distribution $P(u_i \mid \c)$ \DIFdelbegin \DIFdel{.
...
To resolve such dominance problems, we propose a new \emph{geometric smoothing} function \DIFdelbegin \DIFdel{applying }\DIFdelend \DIFaddbegin \DIFadd{which we apply }\DIFaddend to the general WSD equation (\Cref{eq:generalwsd}).
...
%DIFDELCMD < %%%
\DIFdelend We hypothesize that the sharpness of probability estimates from \Cref{eq:generalwsd} is a result of data sparsity, and of a false independence assumption in \Cref{eq:contextprobtrue}. \DIFdelbegin \DIFdel{False independence and training data sparsity cause overly sharp posterior distribution estimates, the is particularly a problem }\DIFdelend \DIFaddbegin \DIFadd{This sharpness is a problem that was particularly noted }\DIFaddend for n-gram language models \cite{rosenfeld2000two}.
...
We use the \DIFdelbegin \DIFdel{AdaGram  }\DIFdelend implementation\footnote{\url{https://github.com/sbos/AdaGram.jl}} provided by \DIFdelbegin \DIFdel{Bartunov et al. }%DIFDELCMD < \parencite{AdaGrams} %%%
...
WordNet includes frequency counts for each word sense based on Semcor . These form a prior for $P(l_i)$.
\DIFdelbegin %DIFDELCMD < 
...
Historical state of the art systems are very different in function by approaching the WSD task by more direct means. %DIF > \parencite{Navigli:2007:STC:1621474.1621480,moro2015semeval,navigli2013semeval}.
}\DIFaddend 
...
As part of our method for refitting\DIFdelbegin \DIFdel{the sense embeddings to their new senses}\DIFdelend , we present a geometric smoothing \DIFaddbegin \DIFadd{to }\DIFaddend overcome the issues presented by \DIFdelbegin \DIFdel{the }\DIFdelend overly dominant senses \DIFdelbegin \DIFdel{probabilities estimatescaused by limited training data}\DIFdelend \DIFaddbegin \DIFadd{probability estimates}\DIFaddend .
