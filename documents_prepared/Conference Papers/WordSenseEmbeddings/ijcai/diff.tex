...
%DIF DEL draft4.tex                        Thu Feb 16 11:58:33 2017
%DIF ADD AligningWordSenseEmbeddings.tex   Fri Feb 17 16:28:35 2017
\usepackage{ijcai17}
...
\makeatother
%DIF < \setlength{\floatsep}{1pt plus 2pt minus 2pt}
%DIF -------
 %DIF > 
%DIF < \setlength{\textfloatsep}{1pt plus 2pt minus 2pt}
\usepackage[font=small,skip=2pt]{caption} %This is by spec in conference guidelines IDK why it is not occurring in the .sty %DIF > 
\textfloatsep 13pt plus 2pt minus 4pt %DIF > 
...
%DIFDELCMD < %%%
\DIFdelend Our contributions are threefold:
...
%DIFDELCMD < %%%
\DIFdelend We show how our techniques improve the performance of the Adaptive Skip-Gram sense embeddings for word similarly evaluation; and how they allow the embeddings to be used for lexical word sense disambiguation -- which was not possible using the induced sense embeddings.
\end{abstract}
...
As each word is only given one embedding\DIFaddbegin \DIFadd{, }\DIFaddend such methods are restricted to \DIFdelbegin \DIFdel{only representing }\DIFdelend \DIFaddbegin \DIFadd{the representation of only }\DIFaddend a single combined sense, or meaning, of the word.
...
It can be argued that many word sense induction (WSI) systems may capture better word senses than human lexicographers do manually\DIFdelbegin \DIFdel{, however }\DIFdelend \DIFaddbegin \DIFadd{.
However, }\DIFaddend this does not mean that induced senses can replace standard lexical senses.
...
We find that in doing so\DIFaddbegin \DIFadd{, }\DIFaddend the sense of the word in that sentence is captured.
...
We demonstrate how the refitting method can be applied for WSD in \Cref{lexicalWSD}.
...
\wordquote{banks} as in \DIFdelbegin %DIFDELCMD < \enquote{the river banks were very muddy.}%%%
\DIFdelend \DIFaddbegin \enquote{the river banks were very muddy}\DIFaddend .
...
The method we propose, using our refitted embeddings, has lower time complexity than AvgSimC \parencite{Reisinger2010}, the current standard measure for \DIFaddbegin \DIFadd{the }\DIFaddend similarity of words in context with embeddings.
...
\Cref{Framework} presents our refitting method, as well as \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{our proposed }\DIFaddend geometric smoothing method\DIFdelbegin \DIFdel{used with it}\DIFdelend .
...
Finally, the paper concludes \DIFdelbegin \DIFdel{with outlook to future works }\DIFdelend in \Cref{conclusion}.
...
Iacobacci et al. \shortcite{iacobacci2015sensembed} use a Continuous Bag of Word language model \parencite{mikolov2013efficient}, using word senses as the labels rather than words.
...
Chen et al. \shortcite{Chen2014} use a semi-supervised approach to train sense vectors.
They partially disambiguate their training corpus, using initial word sense vectors and WordNet; and use these labels to \DIFdelbegin \DIFdel{fine-turn }\DIFdelend \DIFaddbegin \DIFadd{fine tune }\DIFaddend their embeddings.
...
Refitting is a one-shot learning solution, as compared to the supervised and semi-supervised approaches used in \DIFdelbegin \DIFdel{existing works }\DIFdelend \DIFaddbegin \DIFadd{the works discussed above}\DIFaddend .
...
By defining a stochastic map between \DIFaddbegin \DIFadd{the }\DIFaddend induced and lexical senses, Agirre et al. \shortcite{agirre2006}, propose a general method for allowing WSI systems to be used for WSD.
\DIFdelbegin \DIFdel{This is more general than the approach we propose here, which only works for sense embedding based WSI. 
}\DIFdelend Their work was used in SemEval-2007 Task 02 \parencite{SemEval2007WSIandWSD} to evaluate all entries. 
...
\DIFdelbegin \DIFdel{It was evaluated on the small SensEval 3 English Lexical Sample }%DIFDELCMD < \parencite{mihalcea2004senseval}%%%
\DIFdel{.
It is less clear how well this method will work on more complete tasks featuring rarer words and senses}\DIFdelend \DIFaddbegin \DIFadd{This is more general than the approach we propose here, which only works for sense embedding based WSI.
By exploiting the particular properties of sense embedding based WSI systems we propose a system that can better facilitate the use of this subset of WSI systems for WSD}\DIFaddend .
...
During preliminary investigations, we attempted \DIFdelbegin \DIFdel{directly optimising }\DIFdelend \DIFaddbegin \DIFadd{to directly optimise }\DIFaddend the sense vector to predict the example.
We applied the L-BFGS \parencite{nocedal1980updating} optimisation algorithm with the sense vector being the parameter being optimised over, and the objective being to maximise the probability of the example sentence according \DIFdelbegin \DIFdel{the the }\DIFdelend \DIFaddbegin \DIFadd{to the }\DIFaddend language model.
...
Bayes' Theorem can be used to estimate the posterior predictive distribution $P(u_i \mid \c)$ (see \Cref{generalwsd}; or the smoothed variation in \Cref{smoothing}).
...
Using the language model and application of Bayes' theorem, we define a general word sense disambiguation method that can be used for refitting (\Cref{eq:synth}), and \DIFdelbegin \DIFdel{also can }\DIFdelend \DIFaddbegin \DIFadd{can also }\DIFaddend be used for lexical word sense disambiguation (see \Cref{lexicalWSD}).
...
Given this, we have \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend estimate of $P(\c \mid s_{i})$ which can be used to find $P(s_i \mid \c)$.
...
To resolve such dominance problems, we propose a new \emph{geometric smoothing} function which we apply to the general WSD equation (\Cref{eq:generalwsd}).
...
Further to this, the frequency of words \parencite{zipf1949human}  and word senses \parencite{Kilgarriff2004} within a corpus both follow \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend approximate power law distribution (Zipf's Law).
...
Thus handling our issues with overly sharp posterior estimates.
...
The induces sense embeddings from these two models are used to evaluate the performance of our methods on similarity in context (\Cref{SimilarityInContext}) and at word sense disambiguation (\Cref{lexicalWSD}).
...
The vocabulary \DIFdelbegin \DIFdel{is restricted to only }\DIFdelend \DIFaddbegin \DIFadd{used is smaller than for the AdaGram model.
Common }\DIFaddend words with at least \DIFdelbegin \DIFdel{250 occurrences, which results in a vocabulary size of 88,262. Words with at least }\DIFdelend 20,000 occurrences \DIFdelbegin \DIFdel{, }\DIFdelend are giving 20 senses\DIFdelbegin \DIFdel{, and the remainder just }\DIFdelend \DIFaddbegin \DIFadd{.
Words with at least 250 occurrences are restricted to }\DIFaddend a single sense.
\DIFaddbegin \DIFadd{The remaining rare words are discarded. 
}\DIFaddend This results in \DIFdelbegin \DIFdel{the most common }\DIFdelend \DIFaddbegin \DIFadd{a vocabulary size of 88,262, with }\DIFaddend 2,796 words having multiple senses.
\DIFdelbegin \DIFdel{This is not a near-full coverage of the language. 
}\DIFdelend 
...
%DIFDELCMD < %%%
\DIFdelend For each of the target words and contexts; we use refitting on the target word to create a word sense embedding specialised for the meaning in the context provided.
...
\subsection{AvgSimC}
In their seminal work on sense vectors representations\DIFaddbegin \DIFadd{, }\DIFaddend Reisinger and Mooney \DIFaddbegin \shortcite{Reisinger2010} \DIFaddend define a number of measures for word similarity suitable for use with sense embeddings\DIFdelbegin %DIFDELCMD < \parencite{Reisinger2010}%%%
\DIFdelend .
...
\end{multline}
for contexts $\c$ and $\c^\prime$, the contexts of the two words to be compared\DIFdelbegin \DIFdel{.
And }\DIFdelend \DIFaddbegin \DIFadd{, and }\DIFaddend for $\u=\{u_1,...,u_n\}$ and $\u^\prime=\{u^\prime_1,...,u\prime_{n^\prime}\}$ the respective sets of induced senses of the two words.
...
\end{figure}
We define a new similarity measure\DIFaddbegin \DIFadd{, }\DIFaddend RefittedSim, as the distance between the refitted sense embeddings.
...
Reisinger and Mooney \shortcite{Reisinger2010} found optimal results in several evaluations near 50 senses.
...
During evaluation\DIFaddbegin \DIFadd{, }\DIFaddend each context paragraph is \DIFdelbegin \DIFdel{converted to lower case, and }\DIFdelend limited to 5 words to either side of the target word, as in the training.
...
\begin{table}
	...
	\end{adjustbox}
	\caption{Spearman rank correlation $\rho \times 100$  as reported by several methods\label{swscEvery}.
		In this table RefittedSim-S refers to our RefittedSim using smoothing and the AdaGram prior, and \DIFdelbeginFL \DIFdelFL{SU }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{RefittedSim-SU }\DIFaddendFL to using smoothing and a uniform prior.
		AvgSimC is the original AvgSimC without smoothing but with the AdaGram prior.}
\end{table}
...
The method of Chen et al. \shortcite{Chen2014}, has a significant lead on the other results presented.
...
whereas the methods discussed in \Cref{eq:generalwsd,eq:generalwsdsmoothed} consider the more general word sense disambiguation problem, as applicable to disambiguating lexical or induced word senses depending on the inputs.
...
First\DIFaddbegin \DIFadd{, }\DIFaddend refitting is used to transform the induced sense vectors into lexical sense vectors.
...
Then\DIFaddbegin \DIFadd{, }\DIFaddend selecting the correct sense is simply selecting the most likely sense:
\begin{equation}
...
Our results using smoothed refitting, both with AdaGram and Greed Embeddings with backoff, outperform the MSF baseline  \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Navigli:2007:STC:1621474.1621480}  }%DIFAUXCMD
}\DIFaddend -- noted as a surprisingly hard baseline to beat \parencite{Chen2014}.
...
While not directly comparable due to the difference in training data, we note that our Refitted results, are similar in performance, as measured by F1 score, to the results reported by Chen et al \shortcite{Chen2014}.
...
%DIFDELCMD < %%%
\DIFdelend We show how our method can be used to create embeddings to evaluate the similarity of words, given their contexts.
...
%DIFDELCMD < %%%
\DIFdelend We also demonstrate how similar refitting principles can be used to create a set of vectors that are aligned to the meanings in a sense inventory, such as WordNet.
...
%DIFDELCMD < %%%
\DIFdelend As part of our method for refitting, we present a geometric smoothing to overcome the issues presented by overly dominant senses probability estimates.
We show that this significantly improves the performance.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Our refitting method provides effective bridging between the vector space representation of meaning, and the traditional discrete lexical representation.
