...
\Cref{Framework} presents our refitting method, as well as the geometric smoothing method used with it.
...
Finally, the paper concludes with outlook to future works in \Cref{conclusion}.
...
By defining a stochastic map between induced and lexical senses, Agirre et al. \shortcite{agirre2006}, propose a general method for allowing WSI systems to be used for WSD.
...
Using the language model and application of Bayes' theorem, we define a general word sense disambiguation method that can be used for refitting (\Cref{eq:synth}), and also can be used for lexical word sense disambiguation (see \Cref{lexicalWSD}).
...
\begin{table}
	...
	\caption{Spearman rank correlation $\rho \times 100$  as reported by several methods\label{swscEvery}.
		In this table RefittedSim-S refers to our RefittedSim using smoothing and the AdaGram prior, and SU to using smoothing and a uniform prior.
