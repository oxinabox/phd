...
\usepackage[style=trad-abbrv,url=false, doi=false, sorting=none]{biblatex}
\bibliography{master}
...
\begin{document}
\title{A Bayes Theorem Based, One-Shot Learning, Method for Aligning Word Sense Vectors}
\maketitle
...
The key contribution of this work is to suggest a new way to synthesis a word sense embedding given only one example. For lack of a better term we call this *aligning* the word sense vectors, to find a new vector, that lines up with the specific meaning of the word from the example sentence.
...
\subsection{Bayesian Aligned Word Sense Vectors vs AvgSimC}
...
where as with out Aligned Word sense vectors, it is a single distance computed over probability weighed word sense vector.
...
Which we contrast with our method, based on using \Cref{eq:synth} to synthesise a new vector that is aligned to give high likelihood of their contexts, under language model:

\begin{multline}
	\mathrm{AlignedSim}((\s,\c),(s^{\prime},\c^{\prime}))\\
	\begin{aligned}
