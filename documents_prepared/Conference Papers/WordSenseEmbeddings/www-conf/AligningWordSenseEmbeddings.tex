...
A Bayes theorem method for refitting word sense vectors to using a single example;
...
The refitment method proposed in this paper is thus not required for these methods.
...
\subsection{Refitment through a mapping corpus}
...
There is a notable time complexity difference between AvgSimC and ReffittedSim.
...
\caption{Results on SWSC. $\rho$ is Speerman rank correlation between the output similarities from each method and the ground truth of the average rating of from the human annotators. \pdfcomment{TODO: Reformat this table. Remove Smoothing column, call out RefittedSim-Without-Smoothing}} \label{swscres}
\end{table*}


We suggest that no results have been presented prior to our work for AdaGrams evaluated on SCWS,
due to the comparative failure of AvgSimC to produce suitable results on that task. As can be seen in \Cref{swscres} AvgSimC preduces a comparatively 
