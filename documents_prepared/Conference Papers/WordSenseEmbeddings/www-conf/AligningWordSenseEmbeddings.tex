...
\pdfcomment{I should tone this section down. After all out WSD results are not great.}
...
\pdfcomment{Explain Adagram Here?}
...
\caption{Results on SCWS. $\rho$ is Spearman rank correlation between the output similarities from each method and the ground truth of the average rating of from the human annotators. For comparison we include subset of the results from the other indicated papers. \pdfcomment{TODO: There is a new version of this table on my other computer, put it in. That version will still need trimming down.		
		}} \label{swscres}
\end{table*}
...
\pdfcomment{Where to put this sentence?}
