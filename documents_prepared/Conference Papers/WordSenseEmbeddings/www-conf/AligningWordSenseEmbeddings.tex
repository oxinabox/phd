...
In this case, it is the same as replacing $P_S(\c \mid s_{i})$ with the geometric mean of the individual word probabilities -- rather than the 

If the uniform prior is used, then this is simply the geometric mean of the individual word likelihoods.

Whether or not the prior is included in the normalisation has no effect, if the uniform prior is used.

\pdfcomment{I am now thinking I should really bring the Prior, inside the root. and smooth that as well.}
...
Also in both cases subsampling of $10^-5$ was used, and decreasing learning rate starting at 0.25 was used.
