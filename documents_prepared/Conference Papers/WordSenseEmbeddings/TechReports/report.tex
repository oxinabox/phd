...
position)
\item for $\mathbf{s}=(s_{1},...,s_{m})$ the set of word senses.
\item The language model provides: $P(w_{i}\mid s_{i})$ (It actually gives
a log probability, but we will ignore that)
\item we have a prior (possibly just the uniform prior): $P(s_i)
\item we can calculate $P_{\mathbf{c}}(\mathbf{c}|s_{i})=\prod_{i=1}^{i=n}P(w_{i}|s_{i})\,P(s_i)$, 
...
approach. The output word sense $l^{\star}=\argmax_{i\in[0,m]}P_{s}(s_{i}^{\ast}\mid\mathbf{c})$ 
\end{itemize}
