...
\begin{frame}{We can word sense induction method which generate word-sense vectors}
	\begin{itemize}
		\item Word sense induction methods \alert{discover the senses} as it trains their vectors.
...
\begin{frame}{For our evaluations we consider two word sense induction models}
	\begin{columns}[T]
		\begin{column}<1->{0.5\textwidth}
			\alert{Greedy}
			\begin{itemize}
				\item Model a \alert{fixed number} of senses vectors
...
\begin{frame}{Induced sense embeddings don't produce standard dictionary senses}
	 \begin{itemize}
	 	...
	 	\note[item]{Most humans learn word senses this way also -- most people don't sit down with a dictionary}
	 	...
	 	\end{itemize}
	 	\item<3-> Not certain to have any sense ideal for \alert{any particular use}.
		\item<4-> \alert{Not interoperable} with lexical knowledge bases. ImageNet etc.	
	 \end{itemize}
\end{frame}
...
\begin{frame}{We want to align these induced senses to a known meaning.}
 \begin{itemize}
 	...
 	\begin{itemize}
 		\item Kid as in \emph{\alert{The fluffy goat kid.}}
 		\item or; Kid as in \emph{\alert{My brother is such an annoying little kid.}}
 	\end{itemize}
	 \item The \alert{listener} immediately knows what is meant.
	 \item We want a \alert{system} that can do that.
 \end{itemize}	
\end{frame}
	...
	\begin{columns}[T]
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				...
				\item We use the \alert{probabilities} of induced senses.
				\item The new embedding is aligned to the meaning in \alert{that sentence}.
			\end{itemize}			
		\end{column}
...
\begin{frame}{The probabilities are found using Bayes'~theorem}
	...
	\begin{columns}[T]
		\begin{column}{0.5\textwidth}
			...
			\onslide<1->{\alert{Sense}-Language model: $P(w_j \mid u_{i})$\\}
			\vspace{1em}
			\onslide<2->{
...
\section{Similarity with Context}
...
\section{Lexical Word Sense Disambiguation}
	...
	\begin{columns}[T]
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				...
				\item The \alert{geometric smoothing} to an extent trades-off between he prior (which is linked to MFS).
				\note[item]{Tweaking the geometric smoothing as a hyperparameters could get a better trade-off, no doubt}
...
\begin{frame}{Conclusion}
	...
	%	\begin{column}{0.5\textwidth}
			\begin{itemize}
				\item \alert{Refitting} constructs new sense embeddings using a single example.
				\item \alert{RefittedSim}, is faster and has higher correlation with human judgement than \alert{AvgSimC}. 
