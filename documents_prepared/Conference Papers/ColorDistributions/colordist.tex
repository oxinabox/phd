...
For what we will call the \emph{Empirical Model} this a sum of the observations from the training case, featuring the same blurring, as in the GRU Model.
The Empirical Model is inferior in generalisability to the GRU model, as it can only handle color names from the training set. 
...
During testing on the development dataset, we found that the empirical model would occasionally return a predicted probability of zero for one of the observations.
This causes the perplexity to be undefined, or $$\inf$$ when evaluated using IEEE Floating point math.
To handle this we propose an additional variant of the Empirical model featuring Add-one, or Laplace Smoothing.
In this, we add an number additional observations to each color name, prior to taking the mean.
We add an extra element for each output bin, with a probability mass of one.
This is a very common technique in language modelling, 
but here we apply it to smoothing the discretized probability estimates in color-space.
...
Shown in \Cref{figyellow}, \Cref{figgreenishyellow}, \Cref{figgreenish}, and earlier in \Cref{grey}, \Cref{figlightgrey}, \Cref{figdarkgrey}, \Cref{figverylightgrey}, \Cref{figverydarkgrey} are side by-side comparisons of the output of the 64 bin GRU model, compared to the Empirical Model, both with blurring factor of $\frac{1}{128}$.
