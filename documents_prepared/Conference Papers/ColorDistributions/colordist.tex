...
More recent works function with much larger number of colors, larger vocabularies, and are larger pools of respondents. They also generally feature more sophisticated compositional understanding.
...
We write $P_{}(y_{1}<Y\le y_{2}\mid Y\sim\mathcal{D}(\mu,\sigma^{2}))$ to mean the probability of a value distributed according to $\mathcal{D}(\mu,\sigma^{2})$ being in the patch bordered by $y_1$ and $y_2$.
...
We are unable to use the work of \textcite{DBLP:journals/corr/KawakamiDRS16} or \textcite{DBLP:journals/corr/MonroeHGP17} for direct comparison,
as neither aims to produce a general distribution.
