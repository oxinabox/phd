...
This allows the prediction of  distributions for color terms combinations never seen in training.
...
It allows the likelihood of the color to be part of determining the confidence in the recognition
\end{abstract}
...
Because of its difficulty, recent state of the art systems for image generation has demonstrated their capacity by generating from texts containing complex color descriptions such as ``the flower has petals that are bright pinkish purple with white stigma'' \parencite{reed2016generative, 2015arXiv151102793M}.
It is a challenging domain, due to ambiguity, multiple roles taken by the same words, many modifiers, and shades of meaning.
In our work the core focus is on the on the linguistic phenomena around the descriptions of the color of a single patch as characterized by a color space such as HSV \parencite{smith1978color}.
Issues of illumination and perceived color based on visual context are taken out of the scope.


Understanding color distributions has a clear use as a subsystem in many systems.
...
For example the word \texttt{``tan''} may mean a number of colors for different people in different circumstances ranging from the bronze of a tanned sunbather, to the brown of tanned leather;
...
However, based on knowledge of the population's use of the words  is is possible to find a probability distribution as to the color intended.
...
Thus proper understanding cannot simply map a name to a single point in color space, it must estimate a probability distribution for all of color space.
...
Proper understanding of color language first requires a model capable of understanding linguistic compositionality.
...
for a large number of word combinations there are few example in any given corpus.
...
\textcite{menegaz2007discrete} partitioned the color space using Delaunay triangulation and assigned probabilities for each name to the vertices  based on responses of 6 subjects to 424 color patches.
...
Note also that we are working with 3 orders of magnitude more data.
\textcite{van2009learning} learns a generative topic based model from real world images, rather than color patches.
They use a probabilistic latent semantic analysis based model which is restricted to be unimodal -- which is correct for the small set of colors being considered.
...
The base listener model is of relevance to the distribution estimation task.
...
For example, based on the learned understanding of \texttt{``bright''}, from examples like \texttt{``bright green''} and \texttt{``bright red''}, and of \texttt{``salmon''}, our system can suggest the distribution in the color space of \texttt{``bright salmon''}, even though that description never occurs in the training data.
...
Tagging each token with a lingustic roll as a preproessing step
