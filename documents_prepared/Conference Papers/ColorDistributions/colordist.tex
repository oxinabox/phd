...
This causes the perplexity to be undefined ($$\inf$$ when evaluated using IEEE Floating point math.)
...
%\pgfplotstableread{results/extrapo.tsv}\resextrapo
...
we compare a GRU model trained and evaluated on the Extrapolation dataset
 -- that is to say the set where the rare color descriptions do not show up in training, only in testing --
 to a GRU and a Baseline model that are trained on the full dataset, but evaluated on the Extrapolation test set.
