...
\begin{frame}{Why it wins is now clear}
	\begin{itemize}
		...
		\end{itemize}
		\item Language models are really effective
		\begin{itemize}
			\item If a language model based system has the right words, it will be creating them in the right order.
			\item Even the very basic language model I showed could do that.
		\end{itemize}
	\end{itemize}
...
\begin{frame}{Some might say the problems we are assessing on are not sufficiently difficult}
	...
	\begin{itemize}
		\item The problems are exactly as difficult as they are. \note{Tautology}
		\begin{itemize}
			\item real world problems on real data.
		\end{itemize}
		\item They are being constantly affirmed: 
		\begin{itemize}
			\item if you are doing anything like \emph{text classification} SOWE is all you need.
			\item \citehere{arora2016simple}
		\end{itemize}
	\end{itemize}
\end{frame}
