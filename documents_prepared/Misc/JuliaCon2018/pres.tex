...
\begin{frame}{Australia, it is quiet far away}
	\note{I believe perth is exactly 12 hours out of sync with Boston.}
...
\begin{frame}[fragile]{DataDeps.jl is best for Data with the following properties.}
	\Large
	\begin{columns}
		\begin{column}{0.5\textwidth}
			\structure{DataDeps.jl\\}
			Static\\
			Public\\
			File-Based\\
			Any Format\\
			Any Size\\
		\end{column}
		\begin{column}<2->{0.5\textwidth}  %%<--- here
			\structure{Git\\}
			Dynamic\\
			Public or Private\\
			File-Based\\
...
\begin{frame}[fragile]{\textcolor{Red}{Theoretical}, handling private data, with specific \inlinecode{fetch_method} method}
		\small
		\ttfamily
		\color{Aquamarine}
		using HTTP, OAUTH2\\
		baseurl="https://\ldots/files"; authurl="https://\ldots/auth"; 
		appid="\ldots"; appsecret="\ldots"\\
		\color{Magenta}
		function authed\_download(fileid, local)\\
		\verb!    !authtok = getauthtok(authurl\\
		\verb!        !ENV["GD\_ID"], appid, appsecret)\\
		\verb!    !{url = joinpath(baseurl, fileid}\\
		...
		\color{Green}
		"The data from my google drive of sciency stuff"
		\color{Orange}
		"12fileidencodedinbase64whywecanusepaths21ab", \# fileid\\
		\color{Red}
