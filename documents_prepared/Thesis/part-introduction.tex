...
\section*{Acronyms}
\begin{description}
	\item[Adagram] A type of word sense embeddings
	\item[BOW] Bag of Word
	\item[CBOW] Continous Bag of Words: A (misleadingly named) type of word embeddings
	\item[CNN] Convolutional Neural Network
	\item[FastText] A type of word embedding
	\item[GRU] Gated Recurrent Unit: a form of RNN
	\item[GloVe] A type of word embedding
	\item[LCOWE] Linear Combination of Word Embeddings. A class of methods for combining word embeddings, covering  both SOWE, MOWE and others.
	\item[LSTM] Long Short Term Memory: a form of RNN
	\item[MFS] Most Frequent (word) Sense
	\item[MIP] Mixed Integer Programming
	\item[ML] Machine Learning
	\item[MOWE] Mean of Word Embeddings
	\item[NLP] Natural Language Processing
	\item[n-gram] collections of $n$-sequential words
	\item[RNN] Recurrent (sequential) Neural Network
	\item[RvNN] Recursive (tree structured) Neural Network
	\item[PCA] Principle Component Analysis: as used in this work, a form of dimensionality reduction.
	\item[POV] Point of View, the perspective a novel  is told from.
	\item[SkipGram] A type of word embedding
	\item[SOWE] Sum of Word Embeddings
	\item[SVM] Support Vector Machine
	\item[URAE] Unfolding Recursive  Auto-encoder
    \item[WSD] Word Sense Disambiguation
\end{description}
...
as discussed in \Cref{SentVecMeaning} and \Cref{ColorEst},
...
To further understand the relationship between SOWE and BOW, and the extent to which word order matters, \Cref{BOWgen} and \Cref{SOWE2Sent} investigate if it is possible to reverse the conversion from sentence to SOWE.
The results in \Cref{BOWgen} show that it is largely possible to reconstruct bags of words from SOWE, suggesting that when considered as a dimensionality reduction technique SOWE does not lose much information.
...
\Cref{LitRev} contains significantly more details on each of these.
Each chapter in \Cref{Publications} also reintroduces its own key background information.
...
-- the encoding.
These are useful for natural language understanding tasks, including the color problem in \Cref{ColorEst}.
...
\Cref{Publications} contains investigations on how LCOWE perform in key NLP tasks.
...
\subsection*{Overview of Novel Contributions (\Cref{Publications})}
...
this leads to the investigation into if weighted sums of word sense embeddings might better resplendent a particular usage of a word in \Cref{RefittingSenses}.
...
We conclude with a pair of complementary works in  \Cref{BOWgen,SOWE2Sent}, which investigate the ability to recover bags of words and sentences, from SOWE represented sentences.
...
There exists a trivial extension of the work presented in \Cref{BOWgen,SOWE2Sent}, where the mixed integer programming model is constrained to give the second (and so forth) most likely solution, together with its probability.
...
\Cref{SentVecMeaning} and \Cref{ColorEst} both consider representing contiguous linguistic structures, in particular sentences and shore phrases.
\Cref{SentVecMeaning} directly explores the ability to find regions of the representation space that match to regions of the meaning space.
Further, the output of the input modules discussed in \Cref{ColorEst} are (once trained) points in a grounded representation space, though that work did not examine it directly.
\Cref{RefittingSenses} considers the representation of word senses, and it navigates the representation space to find new representations which better describe a particular use of a word.
