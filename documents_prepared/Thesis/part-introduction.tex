...
With the demonstrated utility of linear combinations of embeddings for representing larger structures than words,
it is worth investigating their utility for the internal structures of words.\todo{This is not right, I don't mean morphemes, I mean senses.}
When it comes to representing word senses, it may be desirable to find a representation of the exact sense of a word being used in a particular example.
...
We call the process of inducing the fine grained senes embedding refitting.
...
AvgSimC is a probability weighted set of similarity measures.
Refitted sim is a similarity measure of a 
\todo{Finish this}


We define a method to generate new word sense embeddings from an existing set of sense embeddings using a single example of use.


by using their a linear combination of the existing embeddings with weight based on how likely those senses are to occur in an example sentence.
It finds the sense embeddings generated this way are more useful for similarity comparison, but not competitive for use in word sense disambiguation.
