...
\Cref{LitRev} contains significantly more details on each of these, and more.
Each chapter in \Cref{Publications} also reintroduces its own key background.
...
Secondly, the embeddings are often pretrained on another task, for which there exists plentyful training data.
...
Traditional language modelling, involves using preceding words to predict the next work, as was implemented in \citet{NPLM}.
Advancements on this, to create better embeddings,
is the continuous bag of words (\textbf{CBOW}) which uses the context words from both side of a word to predict it;
and its complement the \textbf{SkipGram}, which uses a single word to predict all words occurring it its context \citep{mikolov2013efficient,mikolov2013linguisticsubstructures}.
