...
\citet{iacobacci2015sensembed} use a Continuous Bag of Word language model \parencite{mikolov2013efficient}, using word senses as the labels rather than words.
...
\citet{Chen2014} use a supervised approach to train sense vectors, with an unsupervised WSD labelling step.
...
By defining a stochastic map between the induced and lexical senses, \citet{agirre2006}, propose a general method for allowing WSI systems to be used for WSD.
...
\citet{NPLM} describe a similar method to \Cref{eq:synth} for finding  (single sense) word embeddings for words not found in their vocabulary.
...
This is well known to occur for n-gram language models \citep{rosenfeld2000two}.
...
During training we use the Wikipedia dataset as used by \citet{Huang2012}.
...
%\subsection{AvgSimC}
\citet{Reisinger2010} define a number of measures for word similarity suitable for use with sense embeddings.
...
\citet{Reisinger2010} found optimal results in several evaluations near 50 senses.
...
The method of \citet{Chen2014}, has a significant lead on the other results presented.
...
We use the weighted mapping method of \citet{agirre2006}, (see \Cref{mapping}) as a baseline alternative method for using WSI senses for WSD.
...
Our results using smoothed refitting, both with AdaGram and Greed Embeddings with backoff, outperform the MFS baseline  \citep{Navigli:2007:STC:1621474.1621480}  -- noted as a surprisingly hard baseline to beat \parencite{Chen2014}.
...
While not directly comparable due to the difference in training data, we note that our Refitted results, are similar in performance, as measured by F1 score, to the results reported by \citet{Chen2014}.
