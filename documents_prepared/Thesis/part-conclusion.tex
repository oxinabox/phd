...
Though some reourderings, such as noun swaps of nouns with similar ontological classification (e.g. Agents, Objects) do exist at all lengths.



nor is any linguistic structure information directly injected.
...
While we would not normally think of the SOWE sentence representation space as one for which there would be an easy alignment to the idea space,
\Cref{SentVecMeaning} showed that it was.
A strong point in its favor is that it directly benefits from word embeddings.
Word embeddings are excellently suited for creating a space which, while ungrounded, has the internal consistancy that makes it very useful as a representation space for supervision to be applied on top of.
...
In this case the idea space is very well grounded to the HSV color space.
We can, for many purposes, say for this natural language understanding task, the color space \emph{is} the idea space.
Using point estimation it outputs a point in idea space, reflecting (in some sense) the most reasonable guess of the meaning.
Using distribution estimation it outputs a distribution over the idea space, fully reflecting the knowledge we have to infer the meaning.
