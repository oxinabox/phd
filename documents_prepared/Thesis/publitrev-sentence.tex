...
\preamble{This chapter originally appeared as Chapter 5 of the book ``Neural Representations of Natural Language'', published by by Springer.\copywarn{}}
...
\aside[Word Sense Embeddings in Sentence Embeddings]{
	While \Cref{sec:word-sense-representations} was all about sense embeddings, they are unmentioned here.
...
The majority of this section draws on the recurrent neural networks (RNN) as discussed in Chapter 2 of \tcite{NRoNL}.
...
\begin{figure}
	\caption{The VAE plus encoder-decoder of \textcite{Bowman2015SmoothGeneration}.
		...
		The Emb. step represents the embedding table lookup. In the diagrams for \Cref{sec:word-representations}  we showed this as  as a table but just as a block here for conciseness.
	}
	\label{fig:bowman}
	...
	\resizebox{\textwidth}{!}{\begin{tikzpicture}[]
		...
		\node(e1)[layer, below=of L1] {\small Emb.};
		\draw[->] (e1) edge node[labe]{$\i C_{\n w_1}$} (L1);
		\node(w1)[below = of e1]{$\n w_1$};
		\draw[->] (w1) -- (e1);
		
		\node(L2)[dotted, layer, right = of L1] {$\ldots$};
		\node(e2)[layer, below=of L2, dotted] {\small Emb.};
		\draw[->, dotted] (e2) edge (L2);
		\node(w2)[below = of e2]{$\ldots$};
		\draw[->, dotted] (w2) -- (e2);
		
		\node(L3)[layer, right = of L2] {$\mathrm{RU_E}$};
		\node(e3)[layer, below=of L3] {\small Emb.};
		\draw[->] (e3) edge node[labe]{$\i C_{\n w_n}$} (L3);
		\node(w3)[below = of e3]{$\n w_n$};
		\draw[->] (w3) -- (e3);
		...
		\node(y5)[above = of L5]{$\n \hat{y}_1$};
		\draw[->] (L5) -- (y5);
		\node(e5)[layer, below=of L5] {\small Emb.};
		\node(w5)[below = of e5]{$\n r_1$};
		\draw[->] (e5) edge node[labe]{$\i C_{\n r_1}$} (L5);
		\draw[->] (w5) -- (e5);
		
		\node(L6)[dotted,layer, right = of L5] {$\ldots$};
		\node(y6)[dotted,above = of L6]{$\ldots$};
		\draw[dotted,->] (L6) -- (y6);
		\node(e6)[layer, below=of L6, dotted] {\small Emb.};
		\draw[->, dotted] (e6) edge (L6);
		\node(w6)[dotted,below = of e6]{$\ldots$};
		\draw[->, dotted] (w6) -- (e6);
		...
		\node(y7)[above = of L7]{$\n \hat{y}_m$};
		\draw[->] (L7) -- (y7);
		\node(e7)[layer, below=of L7] {\small Emb.};
		\node(w7)[below = of e7]{$\n r_m$};
		\draw[->] (e7) edge node[labe]{$\i C_{\n r_m}$} (L7);
		\draw[->] (w7) -- (e7);
...
As described in Chapter 2 of \tcite{NRoNL}, the prompt used for the decoders includes the previous word, concatenated to the code (from the encoder output).
...
\begin{figure}
	...
	\resizebox{0.7\textwidth}{!}{\begin{tikzpicture}[]
		...
		%TOP
		\node(LN5)[layer, above right = 2 of L3, yshift=6cm] {$\mathrm{RU_N}$};
		\node(wN5)[below = of LN5]{$[\v z; \i C_{\n q_0}]$};
		\node(yN5)[above = of LN5]{$\n \hat{q}_{1}$};
		\draw[->] (LN5) -- (yN5);
		\draw[->] (wN5) -- (LN5);
		\path[draw, ->] (L3.east) to[bend left=10] (wN5);
		\node(eN5)[draw, below = of wN5]{\small Emb.};
		\node(wwN5)[below = 2 of eN5]{$\n q_{0}$};
		\draw[->] (eN5) edge node[labe, right]{$\i C_{\n q_{0}}$} (wN5);
		\draw[->] (wwN5) -- (eN5);
		...
		\draw[dotted,->] (LN6) -- (yN6);
		\draw[dotted, ->] (wN6) -- (LN6);
		\draw[dotted, ->] (L3.east) to[bend left=10]  (wN6);
		\node(eN6)[dotted, draw, below = of wN6]{\small Emb.};
		\node(wwN6)[below = 2 of eN6]{$\ldots$};
		\draw[->, dotted] (eN6) -- (wN6);
		\draw[->, dotted] (wwN6) -- (eN6);
		
		
		\node(LN7)[layer, right = of LN6] {$\mathrm{RU_N}$};
		\node(wN7)[below = of LN7]{$[\v z; \i C_{\n q_{\d m_N - 1}}]$};
		\node(yN7)[above = of LN7]{$\n \hat{q}_{\d m_N}$};
		\draw[->] (LN7) -- (yN7);
		\draw[->] (wN7) -- (LN7);
		\path[draw, ->] (L3.east) to[bend left=10]   (wN7);
		\node(eN7)[draw, below = of wN7]{\small Emb.};
		\node(wwN7)[below = 2 of eN7]{$\n q_{\d m_N - 1}$};
		\draw[->] (eN7) edge node[labe, right]{$ \i C_{\n q_{\d m_N - 1}}$} (wN7);
		\draw[->] (wwN7) -- (eN7);
		
		
		%Bottom
		\node(LP5)[layer, below right = 2 of L3, yshift=-6cm] {$\mathrm{RU_P}$};
		\node(wP5)[above = of LP5]{$[\v z; \i C_{\n p_{0}}]$};
		\node(yP5)[below = of LP5]{$\n \hat{p}_{1}$};
		\draw[->] (LP5) -- (yP5);
		\draw[->] (wP5) -- (LP5);
		\path[draw, ->] (L3.east) to[bend right=10] (wP5);
		\node(eP5)[draw, above = of wP5]{\small Emb.};
		\node(wwP5)[above = 2 of eP5]{$\n p_{0}$};
		\draw[->] (eP5) edge node[labe]{$\i C_{\n p_{0}}$} (wP5);
		\draw[->] (wwP5) -- (eP5);
		...
		\draw[dotted,->] (LP6) -- (yP6);
		\draw[dotted, ->] (wP6) -- (LP6);
		\path[draw, ->,dotted] (L3.east) to[bend right=10] (wP6);
		\node(eP6)[dotted, draw, above = of wP6]{\small Emb.};
		\node(wwP6)[above = 2 of eP6]{$\ldots$};
		\draw[dotted, ->] (eP6) -- (wP6);
		\draw[dotted, ->] (wwP6) -- (eP6);
		
		
		\node(LP7)[layer, right = of LP6] {$\mathrm{RU_P}$};
		\node(wP7)[above = of LP7]{$[\v z; \i C_{\n p_{\d m_P - 1}}]$};
		\node(yP7)[below = of LP7]{$\n \hat{p}_{\d m_P}$};
		\draw[->] (LP7) -- (yP7);
		\draw[->] (wP7) -- (LP7);
		\path[draw, ->] (L3.east) to[bend right=10] (wP7);
		\node(eP7)[draw, above = of wP7]{\small Emb.};
		\node(wwP7)[above = 2 of eP7]{$\n p_{\d m_P - 1}$};
		\draw[->] (eP7) edge node[labe]{$\i C_{\n p_{\d m_P - 1}}$} (wP7);
		\draw[->] (wwP7) -- (eP7);
		
		
		\node[labe, right = 0 of L3] {$\v z$\\output\\(code)};
...
Recursive autoencoders are autoencoders, just as the autoencoder discussed in Chapter 1 of \tcite{NRoNL}, they reproduce their input.
...
This matrix-vector RNN is an extension of the GRU discussed in  Chapter 2 of \tcite{NRoNL}, but without a reset gate.
