...
\aside[SOWE is a BOW product]{
	...
	For some set of words $\seq{W} = \lbrace{w_1,\ldots, w_n}$:
...
The method discussed in \Cref{sec:word-representations}, can be adpted to use n-grams rather than words as the basic token.
...
\begin{figure}
	...
	The input (being the current sentence) is $\n w_t$.
}
	...
	\resizebox{\textwidth}{!}{\begin{tikzpicture}[]
		...
		\foreach \y/\r/\dd in {q/a/N, p/b/P} {
			\path[draw, ->] (L3.east) edge node[labe] {output} (L\dd5);
					
			\node(w\dd5)[below = of L\dd5]{$\nv \r_{1}$};
			\node(y\dd5)[above = of L\dd5]{$\n \hat{\y}_{1}$};
			\draw[->] (L\dd5) -- (y\dd5);
			
			\node(L\dd6)[layer, right = of L\dd5] {$\ldots$};
			\node(w\dd6)[below = of L\dd6]{$\ldots$};
			\node(y\dd6)[above = of L\dd6]{$\ldots$};
			\draw[->] (L\dd6) -- (y\dd6);
			
			
			\node(L\dd7)[layer, right = of L\dd6] {$\mathrm{RU_\dd}$};
			\node(w\dd7)[below = of L\dd7]{$\nv \r_{\d m_\dd}$};
