...
What sense of a word being used is determined by the context.
...
\pdfcomment{It would be really easy to make really informativive pictures for this. but idk if it is worth the page space for discredited models}
...
A follow up paper, \tcite{mesnil2014ensemble} it was found that reweighed bags of n-grams \pcite{wang2012baselines} out perform the paragraph vector models.
...
\aside[Machine learning frameworks for structural models]{
	Structural networks can not be defined in most modern static neural network frameworks,	such as TensorFlow.
...
That is to say, each word has both an embedding, and a composition matrix.
This means each phrase node must output both an embedding and a matrix also.
\textcite{SocherMVRNN} considered this for binary constituency parse trees.
The options for extending it to Dependency parse trees should be apparent.
...
It is an extension of the GRU discussed in \Cref{sec:rnn}, but without a reset gate.
...
\aside[Remember:]{Product of a matrix with a concatenated vector, is the same as the sum of the two blocks of that matrix each multiplied by the blocks of that vector.}
