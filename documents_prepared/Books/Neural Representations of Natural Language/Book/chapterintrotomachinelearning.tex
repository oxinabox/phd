...
\begin{figure}
	...
	\begin{adjustbox}{max width=\textwidth}
		\begin{tikzpicture}[]
		\node(hidden)[layer]{$h_t = \varphi \left( \sigma \left( V[x_t; s_{t-1} \circ r_t] + d \right) \right)$};
		...
		\node(state)[layer, above = 5 of reset]{$s_t = (1-z_t) \circ h_t + z_t\circ s_{t-1}$};
		
		\node(in)[below = 3 of reset]{input};
		\node(stm)[left = 3 of reset]{old state};
		\node(stp)[right = of state]{new state};
...
GRU is actually a simplification of the much older and better known Long Short Term Memory,
