...
The most basic recurrent unit, would be a single hidden layer, with the unit output, and the unit state both being the value of this layer.
...
One could call this an Elman cell:
the networks considered by in \tcite{elman1990finding} are such a basic RU, with an output layer on-top.
...
since $z_t$ and $r_t$ are themselves very similar to Basic RUs -- though they do not control there own state.
