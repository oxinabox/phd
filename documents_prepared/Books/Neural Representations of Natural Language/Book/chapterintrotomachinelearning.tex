...
\setchapterpreamble{%
	\dictum[J. M. G. Lammens, PhD Dissertation,
...
\begin{figure}
	\caption{A diagram of a neural network, with 2 inputs, and 3 outputs, and 2 hidden layers of width 5 and 6 neurons respectively.}
	\label{fig:nn}
	\resizebox{\textwidth}{!}{
		\begin{tikzpicture}
		\numdef{\lenL}{2}
		...
		\foreach[count=\j from 0] \ii in {1,...,\lenL} {
			\node(L\ii)[below = of L\j] {$\iv x_\ii$};
		}
		\node[labe, above=0.5 of L0]{Input\\ Layer};
		
		\coordinate[right = 5 of L0, yshift=4cm] (M0);
		\foreach[count=\j from 0] \ii in {1,...,\lenM} {
			\node(M\ii)[draw, circle,below = of M\j] {$\i {\d {\v z}_1}_\ii$};
			
			\foreach \k in {1,...,\lenL} {
				\draw[->] (L\k) -- (M\ii);
			}		
		}
		\node[labe, above=0.1 of M0]{1nd\\Hidden Layer};
		
		\coordinate[right = 4 of M0, yshift=1cm] (O0);
		\foreach[count=\j from 0] \ii in {1,...,\lenO} {
			\node(O\ii)[draw, circle,below = of O\j] {$\i {\d {\v z}_2}_\ii$};
			
			\foreach \k in {1,...,\lenM} {
				\draw[->] (M\k) -- (O\ii);
			}		
		}
		\node[labe, above=0.1 of O0]{2nd\\Hidden\\ Layer};
		
		\coordinate[right = 5 of O0, yshift=-4cm] (P0);
		\foreach[count=\j from 0] \ii in {1,...,\lenP} {
			\node(P\ii)[below = of P\j] {$\iv y_\ii$};
			
			\foreach \k in {1,...,\lenO} {
				\draw[->] (O\k) -- (P\ii);
			}		
		}
		\node[labe, above=0.5 of P0]{Output\\ Layer};
...
\asidefig[An example of a softmax activation in use for with 3 classes for \mbox{$z\mapsto\softmax([z; \frac{z}{10}; {-}2z)]$}. Notice at all points the 3 outputs sum to 1.]{
	\begin{tikzpicture}
	\begin{axis}[sideplot, enlargelimits={rel=0.25}, 	domain=-5:5,]
	\addplot+[mark=none, thick, color=blue] {exp(x)/(exp(x)+exp(0.1*x)+exp(-2*x))} node[pos=1, below]{$\i \hat{y}_1$};
	\addplot+[mark=none, thick, color=red!80!black] {exp(0.1*x)/(exp(x)+exp(0.1*x)+exp(-2*x))} node[pos=0, above]{$\i \hat{y}_2$};
	\addplot+[mark=none, thick, color=green!80!black] {exp(-2*x)/(exp(x)+exp(0.1*x)+exp(-2*x))} node[pos=0, below]{$\i \hat{y}_3$};
	\end{axis}
	\end{tikzpicture}
}
...
\label{equ:totalloss}
\end{equation}
\Cref{equ:totalloss} nothing but a nonlinear function to be minimised by adjusting the values of the weights and biases in $\theta$.
...
% Below line is declared in preamble.tex for reasons of how \makenote* works
...
%\asidefig[Recreation of an input from the MNIST test set. Input is on the left, output is on the right. 	\label{fig-ae-recreation}]{
