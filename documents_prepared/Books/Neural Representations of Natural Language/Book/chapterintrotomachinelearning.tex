...
\begin{figure}
	\caption{\label{fig:mnistnetwork}}
...
\aside{
	...
	simply by using a very wide hidden-layer, that was not feasible as the time of those benchmarks.
...
\aside[Time-Step]{ RNNs are normally described in terms of a time-step.
	This is the advancement of the network such that the previous output state, in now the input old-state.
...
In other NLP applications it might actually be words in the reveres order to that is which they are normally said.
...
\aside[Jordan RU]{The formulation is not commonly used today, but were one considering the Elman network as an Basic RU.
	Then similar can be done for a Jordan network \parencite{jordan1986rnnTR}.
