...
It will not cover techniques that are special to natural language processing – those will be discussed in the later chapters.
\end{abstract}
...
\subsubsection{RELU6}
\aside[Why are we bringing up ReLU6?]{ReLU6 is one of the more obscure activation functions. As such it may seem a bizarre inclusion in this shorter introduction. However, we have found it surprisingly successful as an activation function, e.g. in the Auto-encoder example. Further to that, as a function bounded both above and below it is much more theoretically nice than the ReLU unit.}
...
In general an one or more feedforward layers (that is to say nonrecurrent) are often put on top of the outputs that are used, and before the inputs.
...
\begin{figure}
	...
	\begin{adjustbox}{max width=\textwidth}
		...
		\node(state)[layer, above = 5 of reset]{$s_t = (1-z_t) \odot h_t + z_t \odot s_{t-1}$};
		...
		\draw[->, bend right] (stm) edge node[labe] {$s_{t-1}$} (hidden.south);		
		\draw[->] (reset.north) edge node[labe] {$z_t$} (hidden.south);
		\draw[->, bend right] (stm) edge node[labe] {$s_{t-1}$} (hidden.south);
