...
\begin{equation}
y_i=\left(s_m(z)\right)_i=\left( \frac{\exp{z_i}}{\sum_{j=1}^{j=N} \exp{z_j}} \right)
\end{equation}
...
\begin{align} 
P(Y=i\mid Z=z)&=\frac{\exp{[Wz+b]_{i}}}{\sum_{\forall j}\exp{[Wz+b]_{j}}}
&=\frac{\exp{[Wz]_{i}}\,\exp{b_{i}}}{\sum_{\forall j}\exp{[Wz]_{j}}\,\exp{b_{j}}}
\end{align}

One can see that the bias term $\exp{b_{i}}$ does not depend on the value of $z$.
...
Marginalising the probability over all $Z$ values gives $P(Y=i)=\exp{b_{i}}$. \pdfcomment{Shouldn't there be a denominator here?}

\pdfcomment{Is this actually a probability, or is it a score?}

The other component is $\exp{[Wz]_{i}}$.
