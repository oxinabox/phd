...
such as restrict Boltzmann machines, Hopfield networks, self-organizing maps etc.
...
\aside[multinomial logistic regression]{A neural network classifier with a softmax output layer is sometimes called a multinomial logistic regression network (especially if there are no hidden layers).
	...
	For $v=\i{\v z}_2 - \i{\v z}_1$
	This reduces to $s_m(\v z)= \left[ \sigma({v});  1-\sigma(v) \right]$.
}
...
\begin{equation}
\iv y_i=\i {\left(s_m(\v z)\right)}_i= \frac{\exp{\iv z_i}}{\sum_{j=1}^{j=N} \exp{\iv z_j}}
\end{equation}
...
\begin{equation}
P(Y=i\mid \v z) = \i {\left(s_m(\v z)\right)}_i  = \iv y_i
\end{equation}
...
The total loss ($Loss$) is defined by taking the sum over the whole training set ($\s{X}$) of input output pairs.
\begin{equation}
Loss(\tilde{P}, \s X) = \sum_{\forall (\v x, \v y)\in\s{X}} loss(\v y, \v f(\v x),\tilde{P}))
\end{equation}
...
\pdfcomment{See comment in Latex.}
%jekbradbury 
%e.g. "When diagonal preconditioners are applied in an online optimization setting (i.e.  in conjunction with SGD), they are often referred to as adaptive learning rates in the neural network literature." from \url{https://arxiv.org/abs/1502.04390}
%basically what you really want in HF is not gradient descent preconditioned by the inverse Hessian, but gradient descent preconditioned by the inverse of something Hessian-like that's nonetheless positive-definite
%"we see that RMSProp fairly well estimates $\sqrt diag(H)^2$ in the beginning of training, but then quickly diverges"
%in \url{http://colinraffel.com/wiki/stochastic_optimization_techniques#fn__8}, that paper is summarized as " In 8, it is shown that the $\sqrt (gt+1)$ term approximates (in expectation) the diagonal of the absolute value of the Hessian matrix (assuming the update steps are N(0,1) distributed). It is also argued that the absolute value of the Hessian is better to use for non-convex problems which may have many saddle points. " (edited)
...
\hat{y}=s_m(\d W_2 z + \dv b_2) \\
P(Y=i\mid X=\v x) &= \hat{y}_i \\
&= \left[s_m(\d W_2 \varphi(\d W_1 \v x + \dv b_1)+\dv b_2)\right]_i
\end{align}
...
We will return to this these row vectors  $\i W_{i,:}$ when we discuss as output embeddings in \Cref{sec:word-representations}.
