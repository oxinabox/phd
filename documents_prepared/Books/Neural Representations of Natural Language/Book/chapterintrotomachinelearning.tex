...
\begin{figure}
	...
	\draw[->, bend left] (hidden.north) edge node[labe] {$s_{t}$} (stp);
	\draw[->] (hidden) edge node[labe] {$\hat{y}_t$} (out);
...
\begin{figure}
	\caption{A LSTM Recurrent unit, that an RNN could be made from}
	\label{fig:lstm}
	\begin{adjustbox}{max width=\textwidth}
		\begin{tikzpicture}[]
...
\pdfcomment{Mikolov has a cool paper about these, though this might endup a sidebar}
