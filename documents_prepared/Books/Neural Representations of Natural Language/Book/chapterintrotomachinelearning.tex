...
\begin{abstract}
	...
	It does not cover all aspects of machine learning, or even neural networks, which of-course could be many entire books on their its own.
	It does contain a perhaps surprising amount of detail on recurrent neural networks, and these are very important in natural language tasks.
\end{abstract}
...
The earliest proofs of this was restricted to bounded activation functions like the sigmoid \pcite{mhaskar1992uat} \pcite{}.
...
\end{cases}
\end{equation}
\aside[Why are we bringing up ReLU6?]{ReLU6 is one of the more obscure activation functions. As such it may seem a bizarre inclusion in this shorter introduction. However, we have found it surprisingly successful as an activation function, e.g. in the Auto-encoder example which follows.}
...
\aside[Evaluation function vs Loss function]{
	...
	The loss function is applied to the output of the network during training to calculate the error between the network output for a given input, and the target output from the example.
}
...
We suggest there is an interesting theoretical space about the relationship between adapting the learning rate, and implicitly finding the second derivative (the Hessian), as in the more traditional quasi-Newtonian methods.}
...
\begin{equation}
\argmin_{\tilde{P}} loss_{total}(\tilde{P}, X_{train}, Y_{train})
\end{equation}
...
\aside[What is back-propagation?]{%
...
\pdfcomment{I am tempted to make this a chapter of its own}
...
\aside[Time-Step]{ RNNs are normally described in terms of a time-step.
	This is the advancement of the network such that the previous output state, is now the input old-state.
	...
	For example, using laser distance imaging to detect patterns of wear on tyres of differing sizes,
	each time-step is a different part of the tyre.
}
...
\pdfcomment{better example for non-temporal timestep}
...
In general an one or more feed-forward layers (that is to say non-recurrent) are often put on top of the outputs that are used, and before the inputs.
These extra layers can be considered part of the RU, or as something that surrounds the RU, only at certain time-steps,
...
\begin{figure}
	\caption{A basic recurrent unit, that an RNN could be made from}
	\label{fig:basicrnn}
	\begin{tikzpicture}[]
...
Due to the vanishing gradient problem. \pdfcomment{I need to discuss what the vanish gradient problem is prior to this; maybe in a sidebar}
...
it it is hard for the network to learn the connection between inputs many time steps ago, and the final correct output, and the error signal gradient may have become very small in the intervening layers/time-steps \pcite{bengio1994learning}.
...
GRU is actually a simplification of the much older and better known Long Short Term Memory (LSTM),
 which will be discussed in \Cref{sec:ltsm}) however we discuss GRU as it is a simpler system.
In the evaluations of \tcite{chung2014empirical} \tcite{jozefowicz2015empirical} it was found to perform very well -- similar to LSTM for most tasks.
...
\aside[Trade-off formula]{It may be stating the obvious, 
	but a standard way (and the only sensible way) to trade-off the value between two possible choices: $a$ and $b$,
...
\begin{figure}
	\caption{A Gated Recurrent unit, that an RNN could be made from}
	\label{fig:gru}
...
\begin{figure}
	\caption{A LSTM Recurrent unit. It may seem complex, and that is basically because it is. Though if can be broken down into understand-able parts}
	\label{fig:lstm}
	\begin{adjustbox}{max width=\textwidth}
		\begin{tikzpicture}[]
		...
		\node(state)[layer, above = 5 of igate]{$c_t =  i_t\odot h_{t-1} + f_t \odot c_{t-1}$};
