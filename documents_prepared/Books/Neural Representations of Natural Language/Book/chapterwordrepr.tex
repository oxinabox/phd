...
\aside[CBOW vs Skip-Gram]{For CBOW the context is used to predict the target word. For skip-gram the target word is to predict the context.}
...
\aside[Skip-gram naming]{In different publications this model may be called skipgram skip-gram, skip-ngram, skipgrams etc. Further, it may be called \texttt{word2vec} after the publicly released implementation of the algorithm. Though that software also can be used for CBOW.}
...
While Skip Grams and CBOW were introduces in the same set of papers, subsequent work has found the SkipGrams are almost always the preferred system for any task. \pdfcomment{Citation for this?}
...
SkipGrams are intrinsically prediction based methods, effectively predicting what words will co-occur in the context of a local window and optimising as such.
...
This can be contrasted as a saturating variant of the effective weighing of skipgrams being $X_{ij}$.
...
found that SkipGrams out-performed GloVE on all tasks, though the performance is relatively comparable.
...
Very similar properties were also explored for skip-grams with negative sampling in \tcite{levy2014neural}.
...
Skip-Gram accomplishing something similar again using the subsampling tweak -- common words are randomly skipped from the contexts during training, based on how common they are.
...
As we have now concluded that neural predictive co-location models are functionally equivalent to matrix factorisation of co-location counts,
