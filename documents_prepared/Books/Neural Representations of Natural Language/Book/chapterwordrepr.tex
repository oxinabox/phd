...
Techniques such as CBOW and skip-gram models (\texttt{word2vec}), and more recent advances in relating this to common linear algebraic reductions on collocations as discussed.
...
However, these models are closer to the end point of of the chapter, and of word-representation techniques, than they are to the beginning.
...
Word embeddings are vector representations of works.
...
P(\n w_i \mid \n w_{i-1}, \ldots, \n w_1) = P(\n w_i \mid \n w_{i-1}, \n w_{i-2})
\end{equation}
More generally, one can use $n$-gram language model where for any value of $n$,
...
This motivates the need for representations that hold such linguistic information.
...
This is exactly the kind of task which a neural network achieves implicitly in it's internal representations.
...
\aside[$\i C_{\n w_i}$ not $\i C_i$]{
Note that here we use the word $\n w_i$ as the index in to lookup the word embeddings, \emph{not} the word's index in the corpus.
...
The model has learnt common features about that words implicitly from how they are used, and can use those to make better predictions.
...
\aside[Consider, that the matrix product of a row vector with a column vector is the dot product]{
	...
	This is particularly relevant for the skip-gram model discussed in \Cref{sec:skip-gram} (with just one input word and not hidden layer).	
}
...
While looking at it this way, we can also consider the Bayes-like reformulation from \Cref{sec:softmax-and-bayes-theorem}:
\begin{multline}
...
\tcite{NPLM} is a more advanced version of the neural language model discussed above.
...
\begin{figure}
	\caption{RNN Language Model. The RU equation shown is the basic RU uses in \textcite{mikolov2010recurrent}.
...
\aside[No Bias?]{It should be noticed that \Cref{equ:rnnlm1,equ:rnnlm2} are missing the bias terms.
	...
	or to the assumption that the equations are given in \emph{design matrix} form: where the bias is not treated as a separate term to weights, and the input is padded with an extra 1.
...
This did indeed offer an improvement over the basic RU network used in \textcite{mikolov2010recurrent}.
...
For the best performing networks of both types the results found were significantly better than reported in \textcite{sundermeyer2012lstm} and \textcite{mikolov2010recurrent}.
...
As it is acausal it cannot be done in a real-time system, and is not useful for as many tasks directly as a normal a language model. 
...
Some would also say they have no hidden layer, though this is incorrect: the projection layer of the embedding is functionally an unusual hidden layer.
...
As these are complicated and often misunderstood topics in their own right are discussed in a more tutorial fashion in \Cref{sec:hierachical-softmax-and-negative-sampling}.
...
\pdfcomment{Maybe assign the context windows words to a sequence, like in the sense embedding chapter, to simplify notation}
\begin{align}
P(\n w_i & \mid \n w_{i-\frac{n}{2}},\ldots, \n w_{i-1}, \n w_{i+1},\ldots, \n w_{i+\frac{n}{2}})  \nonumber
...
While in CBOW each training case has one output to be trained to predict, in skip-gram there are multiple.
...
\begin{align}
P(\n w_j \mid \n w_i) & = \i {\softmax \left(V\,\i C_{\n w_i}\right)}_{\n w_j} \\
P(\n w_j \mid \n w_i) & = \frac{\exp{\i V_{\n w_j,:} \i C_{\n w_i}}}{\sum_{\forall v \in \set V} \exp{\i V_{v,:}\i C_{v}}}
\end{align}
...
\asidefig[\label{fig:anavecs} Vectors involved in analogy ranking tasks, this may help to understand the math in \Cref{equ:analogy}]{
	...
	\draw[->] (d) -- (c) node[labe,midway]{$\i C_d- \i C_c$};
	\end{tikzpicture}}
}
...
the following is computed the using table of embeddings $C$ over the vocabulary $\set V$:
\begin{align}
...
\aside[Subsampling, and weight saturation]{
	...
	This is related to the saturating of the co-occurrence weights as calculated by $f(X)$ used by GloVe.
}
...
\pdfcomment{Check this, should I have a P here? and in the next equation or just an X? is P model output or true probabilities}
then the optimisation criteria becomes minimising the cross-entropy between the true co-occurrence probabilities given by $P(\n w_j\mid \n w_i)$, and the value of the embedding product.
...
\pdfcomment{Consider referring to the vocabulary in the equations}
...
and the more traditional non-negative matrix factorization of collocation counts used in topic modeling.
...
Later in \tcite{cotterell2017SkipgramisEPCA} showed the equivalence to exponential principal component analysis (PCA).
\tcite{wordvecispca} goes on to extent that show that it is a weighted logistic PCA, which is a special case for the exponential PCA.
...
One might suggest a variety of word embeddings to be created from a variety of different matrix factorisations with different weightings and constraints.
...
it was noted by \textcite{mikolov2013efficient}, the for vocabulary $\set{V}$, that:
\begin{itemize}
...
\pdfcomment{this seems right, but idk if it is. I need to work out a way to check this}


\paragraph{First consider a binary tree with just 1 layer and 2 leaves}
The leaves are $\n n_{00}$ and $\n n_{01}$, these leaf nodes each correspond to a word from the vocabulary, which must have size 2, for this toy example.
...
node being the branch given by following the left branch, while $\n n_{01}$
...
As these are leaf nodes, prediction or of the other is the same as the prediction of one or the other of the 2 words in our vocabulary.
...
To notate it, we will use index $0$ as it is the associated with the decision at node $\n n_{0}$.
Thus we call it $\i V_{0,:}$.

\aside[$\i V_{0,:} \v z$ is a dot product]{We mentioned it before, but just as an extra reminder. The matrix product of a row vector like $\i V_{0,:}$  with a (column) vector like $\v z$ is their vector dot product.}

\begin{align}
P(\n n_{00} \mid \v z) &=\sigma(\i V_{0,:} \v z) \\
P(\n n_{01} \mid \v z) &=1-\sigma(\i V_{0,:} \v z)
\end{align}
Note that for sigmoid function: $1-\sigma(x)=\sigma(-x)$
...
\aside[Combining multiplications]{
	...
	However computing  every single decision is beyond what is required for most uses: hierarchical softmax lets us only computing the decisions that are on the path to the word-leaf we which to query.
	...
	In languages/frameworks with slow looping constructs (numpy, R, octave), where calling into suitable library routines is much faster, this may give speed-up;
	even here it is likely to be minor;
	the time may be better spend writing a C extension library to do this part of the program.
...
Recall that CBOW, skip-gram and all other language modelling based word embedding methods are based around about predicting
 $P(\n w_o \mid \n w_i,\ldots, \n w_{j})$ for some words.
For skip-gram that is just $P(\n w_o\mid \n w_i)$.
The term $\n n_{0\n t_{1}\ldots\n t_{L}}$ 
in  $P(\n n_{0\n t_{1}\ldots\n t_{L}}\mid z)$, 
just represents as  path through the tree to its leave node which represent the word 
$\n w_o$. i.e $P(\n n_{0\n t_{1}\ldots\n t_{L}}\mid z)=P(\n w_o\mid z)$.
...
Though the final one is perhaps a key reason for the difficulties in understanding hierarchical softmax.
...
\pdfcomment{Calling the Z set mathbb Z isn't great as that is normally the integers }

In normal softmax, the true probabilities (from $P^\star$) are implicitly given by frequency of training pairs in the training dataset $\seq X$ (which we take as the ground truth).
\begin{equation}
...
Though $\set Z$ can be non-discrete values, as was the whole benefit of using embeddings, as discussed in \Cref{sec:the-neural-probabilistic-language-model}.
...
Though this would throw away the promise of the probability distribution summing to one across all possible words that could be collocated with your $\n w_i$.
...
\aside[Loss Function]{Readers may want to reread \Cref{sec:softmax-loss-function} to brush up on how we use the training dataset as a ground truth probability estimate implicitly when using cross-entropy loss. Noting that the conditioning term for skip-grams is the co-located words as there is no hidden layer.}
%
...
\pdfcomment{should there be an averagesing across all negative samples?}
\begin{equation}
loss(\n w_j,\n w_i) = 
...
Calculating to the total loss over the training set $\seq{X}$:
\begin{multline}
...
This is very similar to principal component analysis (PCA), and like PCA number components to use $d$ is a variable that can be decreased to achieve dimensionality reduction.
...
Similarly, on the sentence embedding side \textcite{zou2013bilingual}, and \tcite{socherDTRNN} train embeddings from different language and modalities (respectively) directly to be near to their partners (these are discussed in \Cref{sec:sentence-representations-and-beyond}).
