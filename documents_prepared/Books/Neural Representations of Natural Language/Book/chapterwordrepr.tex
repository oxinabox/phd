...
\aside[SemHuff]{It can be noted that the huffman encoding scheme specifies only the depth of a given word in the tree. It does not specify the order.
	...
	They found that on language modelling task this improved performance, in the way one would expect.
...
\aside{Combining multiplications}{
	...
	Which is a not at all trivial problem in packing tree nodes elements into a matrix.
...
\aside{How does this related to word vectors?}{
...
So in $P(n_{0,t_{1},...,t_{i}}\mid z,)$, the term $n_{0,t_{1},...,t_{i}}=w_o$, just represented as  path through the tree to its leave node.
And $z$ is the final pre-output layer of the model, encoding in normal language models all the infromation about what comes after $w_{i},...,w{i+1}$.
...
\aside[The gradient calculations]{They are not fun. They never are for back-propagation. We recommend using a framework with automated differentiation, and/or performing gradients checks against one or via finite-differencing.}
...
\aside[Most words don't co-occur]{
	Some simple reasoning can account this as a as a reasonable consequence of Zipf's law \parencite{zipf1949human} and a prior of the principle of indifference, but there is a further depth to it as explained in \textcite{ha2009extending}}
...
It is another abuse of notation to suggest we 
...
Switching in the sigmoid for the probabilities, this is:
\begin{equation}
Loss = \sum_{\forall (w_i,w_j)\in \mathcal{X}} 
\left(\sum_{k=1}^{k=K} \mathbb{E}_{w_k \sim U_{\mathcal{V}}} [\log \sigma(V_{w_k}^T\,C_{w_{i}}) \right)
...
\begin{equation}
Loss = \sum_{\forall (w_i,w_j)\in \mathcal{X}} 
\left(\sum_{k=1}^{k=K} \mathbb{E}_{w_k \sim U_{\mathcal{V}}} [-1 + \log \sigma(V_{w_k}^T\,C_{w_{i}}) \right)
-\log \sigma(V_{w_j}^T\,C_{w_{i}}
\end{equation}
...
Loss = -\sum_{\forall (w_i,w_j)\in \mathcal{X}}
\left(
\log \sigma(V_{w_j}^T\,C_{w_{i}}) + \sum_{k=1}^{k=K} \mathbb{E}_{w_k \sim U_{\mathcal{V}}} [\log \sigma(-V_{w_k}^T\,C_{w_{i}})] 
\right)
\end{equation}
