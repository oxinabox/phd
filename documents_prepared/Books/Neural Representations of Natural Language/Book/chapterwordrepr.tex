...
It was popularised by \tcite{mikolov2013efficient}, together with Negative Sampling (which is discussed in the next section).
...
In this section, we will for brevity ignore the bias component of each decision at each node.
It can either be handled nearly identically to the weight;
or the matrix can be tank in \emph{design matrix form} with an implictly appended column of ones;
or it can actually be ignored in implementation.
The reasoning for ignoring it in implementation is that the bias encoded unigram probabilities.
...
\paragraph{Combining multiplications}
...
into a matrix.
Depending on the speed, vs memory trade off, it may be better to use an actual using a tree structure.
The optimisation of tree structures data is beyond the scope of this book.
