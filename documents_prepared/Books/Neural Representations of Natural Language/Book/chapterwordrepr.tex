...
\tcite{mikolov2013efficient}, which introduced the CBOW and Skip-Gram is also responsible for the popularisation of Hierarchical Softmax and Negative Sampling methods.
...
Hierarchical softmax is correctly attributed to \tcite{morin2005hierarchical}.
It was popularised by \tcite{mikolov2013efficient}, together with Negative Sampling.

\pdfcomment{Explain it in detail here before the tuitorial section}

\aside[TODO]{Say something about \textcite{SemHuff}}

\subsubsection{An incredibly gentle introduction to hierarchical softmax}

\subsubsection{First consider a binary tree with just 1 layer and 2 leaves}
...
about the decision made at $n_{0}$ \textendash{} $P(n_{0})=1$.

We want to know the probability of the next node being the 0th (i.e.
$P(n_{0}\mid z)$ ) or the first (i.e. $P(n_{01}\mid z)$).
...
We will call this vector $V_{0}^{\prime}$ \textendash{} the decision
associated with node $n_{0}$

$P(n_{00}\mid z)=\sigma(V_{0}^{\prime}z)$

$P(n_{01}\mid z)=1-\sigma(V_{0}^{\prime}z)$
...
$P(n_{01}\mid z)=\sigma(-V_{0}^{\prime}z)$

\subsubsection{Now consider 2 layers with 3 leaves}
...
assuming we have reached $n_{00}$already.
...
$P(n_{000}\mid z,n_{00})=\sigma(V_{00}^{\prime}z)$

$P(n_{001}\mid z,n_{00})=\sigma(-V_{00}^{\prime}z)$
...
$P(n_{000}\mid z)=\sigma(V_{00}^{\prime}z)\sigma(V_{0}^{\prime}z)$

$P(n_{001}\mid z)=\sigma(-V_{00}^{\prime}z)\sigma(V_{0}^{\prime}z)$

\subsubsection{Continue this logic}
...
It's probability can be found as: $P(n_{0t_{1}...,t_{L}}\mid z)=P(n_{0t_{1}...,t_{L}}\mid z,\,n_{0t_{1}...,t_{L-1}})\,P(n_{0t_{1}...,t_{L-1}}\mid z)$

$P(n_{0,t_{1}}\mid z)=\sigma\left((-1)^{t_{1}}V_{0}^{\prime}z\right)$

$P(n_{0,t_{1},t_{2}}\mid z,\,n_{0,t_{1}})=\sigma\left((-1)^{t_{2}}V_{0t_{1}}^{\prime}z\right)$

$P(n_{0,t_{1},...,t_{i}}\mid z,\,n_{0,t_{1}...,t_{i-1}})=\sigma\left((-1)^{t_{i}}V_{0,t_{1}...,t_{i-1}}^{\prime}z\right)$

Applying the conditional probability chain rule to get:

$P(n_{0,t_{1},...,t_{i}}\mid z,)=\prod_{i=1}^{i=L}\sigma\left((-1)^{t_{i}}V_{0,t_{1}...,t_{i-1}}^{\prime}z\right)$
...
$P(n_{0,t_{1},...,t_{i}}\mid z,)=\prod_{i=1}^{i=L}\sigma\left((-1)^{t_{i}+1}V_{0,t_{1}...,t_{i-1}}^{\prime}z\right)$

Or to allow it to vary per node.

As in the formulation of \cite{mikolovSkip}

Using $ch(n)$ to represent an arbitrary child node of the node $n$

and using an indicator function $\left\llbracket a=b\right\rrbracket =\begin{cases}
1 & a=b\\
-1 & a\ne b
\end{cases}$

$P(n_{0,t_{1},...,t_{i}}\mid z,)=\prod_{i=1}^{i=L}\sigma\left(\left\llbracket n_{t_{i}}=ch(n_{t_{i-1}})\right\rrbracket V_{0,t_{1}...,t_{i-1}}^{\prime}z\right)$

\subsubsection{Combining multiplications}

If one wants to find both $V_{00}^{\prime}z$ and $V_{0}^{\prime}z$,

then this can be done using matrices simultaneously (Taking advantage
of optimized matrix multiplication routines)

Using $V_{0}^{\prime}$ and $V_{00}^{\prime}$ as row vectors, and
...
Which is a not at all trivial problem in packing tree nodes elements
into a matrix.
