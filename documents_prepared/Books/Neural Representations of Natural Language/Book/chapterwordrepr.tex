...
\aside[Lookup word embeddings: Hashmap or Array?]{
	...
	Theoretically we do not care if the implementation using a String to Vector dictionary (e.g. a hashmap), or a 2D array from which a column is indexed-out (sliced-from) via an integer index representing the word.
...
\aside[One-hot product or Indexed-lookup]{
	...
	for $\nv e_i$ the one-hot vector with a 1 in the $i$th position,
	...
	Of-course $\i C_i=C\,\nv e_i$, however in practical implementation the performance ramifications are huge.
...
\begin{equation}
\softmax(V \dv z_{2}+ \v k) = \dfrac{\exp{V_{w_{i}}^{T}z_{2}+k_{i}}_{i=0}^{i=N}}{\sum_{\forall j}\exp{V_{j}^{T}z_{2}+k_{j}}}
\end{equation} 
...
\begin{align}
P(\n w_i & \mid \n w_{i-1}, \n w_{i-2}) = \softmax( \nonumber
\\  & \quad U_3 \left[ \i C_{\n w_{i-1}};...; \i C_{w_{i-n}}\right] \nonumber
\\  & + V \: \varphi\left( U\left[\i C_{\n w_{i-1}};...; \i C_{w_{i-n}} \right] + b_1\right) \nonumber
\\  & +b_2)
\end{align}
...
\begin{equation}
	h_{i} = \varphi\left( U\,h_{j-1} + \i C_{w_{j-1}} \right)
	\hat{y_i} = \softmax\left(V \, h_{i-1} \right)
...
\\  & = \softmax(V \sum_{j=i+1}^{j=\frac{n}{2}} \left( \i C_{w_{i-j}}+\i C_{w_{i+j}} \right))
\end{align}
...
\begin{equation}
P(\n w_j \mid w_{i}) = \left[\softmax(V\,\i C_{w_{i}}) \right]_{\n w_j} \label{equ:skipgram}
\end{equation}
...
\begin{align}
P(\n w_j \mid w_{i}) & = \left[ \softmax(V\,\i C_{w_{i}}) \right]_{\n w_j} \\
P(\n w_j \mid w_{i}) & = \frac{\exp(V_{\n w_j}^\prime\,\i C_{w_{i}})}{\sum_{k=1}^{k=N} \exp(V_k^\prime\,\i C_{k})}
\end{align}
\pdfcomment{Check math and if required change prime to transpose.}

The key term here is  $V_{\n w_j}^\prime\,\i C_{w_{i}}$:
the remainder of the expression is normalising this into a probability.
Maximising the probability $P(\n w_j \mid w_{i})$ is equivalent to maximising the dot produce between $V_{\n w_j}^\prime$, the output embedding for $\n w_j$ and  $\i C_{\n w_i}$ the input embedding for $\n w_i$.
...
This is given by 
\begin{equation}
sim(C_u, C_v)=\frac{\i C_{u}\cdot \i C_{v}}{\left\Vert u\right\Vert \left\Vert v\right\Vert }
\end{equation}
...
For each word co-occurrence of $\n w_i$ and $\n w_j$: they attempting to find optimal values for 
the embeddings $\i C_{w_{i}}$, and $V_{\n w_j}^\prime$, while and biases $b_i$ and $k_j$
such that:
$V_{\n w_j}^\prime\,\i C_{w_{i}} + b_i + k_j \approx \log(X_{ij})$.
This is done via the minimisation of 
\begin{equation}
Loss = - \sum_{\forall \n w_i}  \sum_{\forall \n w_j} f(X_{ij})\,V_{\n w_j}^\prime\,\i C_{w_{i}}+b_i+k_j-\log (X_ij)
\end{equation}
...
\begin{align}
P(\n w_j \mid w_{i}) & = \left[\sigma(V\,\i C_{w_{i}}) \right]_{\n w_j} \\
P(\n w_j \mid w_{i}) & = \sigma(V_{\n w_j}^T\,\i C_{w_{i}})
\end{align}
...
\begin{equation}
Loss = \sum_{\forall (\n w_i,\n w_j)\in \mathcal{X}} 
\left(\sum_{k=1}^{k=K} \mathbb{E}_{w_k \sim D_{\mathcal{V}}} [\log \sigma(V_{w_k}^T\,\i C_{w_{i}}) \right)
-\log \sigma(V_{\n w_j}^T\,\i C_{w_{i}}
\end{equation}
...
\begin{equation}
Loss = \sum_{\forall (\n w_i,\n w_j)\in \mathcal{X}} 
\left(\sum_{k=1}^{k=K} \mathbb{E}_{w_k \sim D_{\mathcal{V}}} [-1 + \log \sigma(V_{w_k}^T\,\i C_{w_{i}}) \right)
-\log \sigma(V_{\n w_j}^T\,\i C_{w_{i}}
\end{equation}
...
Loss = -\sum_{\forall (\n w_i,\n w_j)\in \mathcal{X}}
\left(
\log \sigma(V_{\n w_j}^T\,\i C_{w_{i}}) + \sum_{k=1}^{k=K} \mathbb{E}_{w_k \sim D_{\mathcal{V}}} [\log \sigma(-V_{w_k}^T\,\i C_{w_{i}})] 
\right)
\end{equation}
