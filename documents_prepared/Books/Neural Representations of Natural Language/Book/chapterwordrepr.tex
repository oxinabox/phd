...
While looking at it this way, we can also consider the the Bayes-like reformulation from \Cref{sec:softmax-and-bayes-theorem}:
\begin{multline}
...
\aside[No Bias?]{It should be noticed that \Cref{equ:rnnlm1,equ:rnnlm2} are missing the bias terms.
	...
	it is expressly noted that biases were not used in the network as it was not found that they gave a significant improvement to the result.
...
the following is computed using table of embeddings $C$:
\begin{align}
\qquad & \argmax_{\forall d} sim(\i C_d - \i C_c, \i C_a - \i C_b)\\
\text{i.e} & \argmax_{\forall d} sim(\i C_d, \i C_a - \i C_b + \i C_c)
\end{align}
...
it was noted by \tcite{mikolov2013efficient}, the for vocabulary $\set{V}$, that:
\begin{itemize}
...
\aside[$\i V_{0,:} \v z$ is a dot product]{We mentioned it before, but just as an extra extra reminder. The matrix product of a row vector like $\i V_{0,:}$  with a (column) vector like $\v z$ is their vector dot product.}
...
\aside[Combining multiplications]{
	...
	as on most hardware (and BLAS systems) matrix products with $N$ columns, takes a similar amount of time to $N$ vector dot products take similar times.
	...
	even here is is likely to be minor;
...
Recall that CBOW, skip-gram and all other language modelling based word embedding methods are based around about predicting $P(w_o\mid \n w_i\ldots,w{j})$ for some words.
For skip-gram that is just $P(\n w_o\mid \n w_i)$.
The term $\n n_{0\n t_{1}\ldots\n t_{L}}$ in  $P(\n n_{0\n t_{1}\ldots\n t_{L}}\mid z)$, just represents as  path through the tree to its leave node which represent the word $\n w_o$. i.e $P(\n n_{0\n t_{1}\ldots\n t_{L}}\mid z,)=P(\n w_o\mid z,)$.
And $z$ is the final pre-output layer of the model.
In normal language models $z$ encodes all the information about what the model knows of predictions based on $\n w_i\ldots,w{j}$ -- $z$ is thus a proxy term in the conditional probability for those words.
...
Loss_{\n w_j,\n w_i} =  P(\n w_k\mid \n w_i) - \log P(\n w_j\mid \n w_i)
\end{equation}
\pdfcomment{I want to rewrite the math in this section even more still}
%
