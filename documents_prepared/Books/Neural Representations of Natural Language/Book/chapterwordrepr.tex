...
\aside[Word Vector or Word Embedding?]{Some literature uses the term \emph{word~vector}, or \emph{vector-space model} to refer to representations from LDA and LSA etc.
	...
	Word embeddings are vectors, in anycase.
}
...
\aside[Random Variable Naming Convention]{We follow convention that capitalised $W_i$ is a random variable, and $w_i$ is a particular value which $W_i$ may take. The probaility of it taking that value would normally be written $P(W_i{=}w_i)$.
We simply write $P(w_i)$ to mean the same thing.
...
\aside[Google n-gram corpora]{Google has created a very large scale corpora of 1,2,3,4, and 5-grams from over $10^{12}$ words from the the Google Books project. It available without cost from \url{https://books.google.com/ngrams/datasets} \parencite{lin2012syntactic}.
...
\aside[$C_{w_i}$ vs $C_i$]{
Note that here we use the word $w_i$ as the index in to lookup the word embeddings, \emph{not} the words index.
That is to say that if the $i$th word, and the $j$th word are the same: i.e $w_i = w_j$, then they will index out the same vector from $C$.
$w_i = w_j \implies C_{w_i} =  C_{w_j}$.
}
...
Such that $\hat{y}_{w_i} = P(w_i \mid w_{i-1}, w_{i-2})$.
...
\aside[Confusing notation]{Here we hit an ambiguity in the notion.
	...
	$\left[ \hat{y_i} \right]_{w_i}$ is the indexing operation applied to that vector,
	to get back the predicated probability for the $i$th word (i.e. $w_i$).
}
...
\aside[Distance weighted co-occurrence and dynamic window sizing]{
	...
	Pennington et. al. achieve a similar effect by weighting co-occurrences within a window inversely proportionally to the distance between the word. That is to say if $w_i$ and $w_j$ occur in the same window (i.e. $|i-j|<n$), then rathar than contributing 1 to the entry in the co-occurrence count $X_{w_i, w_j}$, they contribute $\dfrac{1}{|i-j|}$.
}
...
\aside[$1-\sigma(x)=\sigma(-x)$]{
	...
	\begin{align}
		1-\sigma(x)&=\frac{e^{x}+1}{e^{x}+1}-\frac{e^{x}}{e^{x}+1}\\
		&=\dfrac{1}{e^{x}+1}\frac{e^{-x}}{e^{-x}}\\
		&=\dfrac{e^{-x}}{1+e^{-1}}\\
		&=\sigma(-x)		
	\end{align}
}
...
In skip-gram there is no hidden layer, and it is just $z=C_{w_i}$,
and the model defines its probability output by $P(w_o\mid w_i) = P(w_o\mid C_{w_i})$.
}
