...
\begin{figure}
	\caption{The unrolled structure of an RNN for us in (a) Matched-sequence (b) Encoding, (c) Decoding and (d) Encoding-Decoding (sequence-to-sequence) problems. RU is the recurrent unit -- the neural network which reoccurs at each time step.}
...
\aside[Time-Step]{ RNNs are normally described in terms of a time-step.
	...
	This doesn't have to be literal time -- indeed it cannot be, as it is discrete.
...
For example \natlang{record} is both a noun and a verb (it is in-fact nearly ubiquitous that all verbs have a noun form).
...
\aside[Pseudo-tokens]{
	...
	As mentioned, outputting \natlang{<EOS>} is required to be known when the decode is done outputting.
...
The encoder-decoder RNN is the generalised structure for sequence to sequence learning \pcite{cho-EtAl:2014:EMNLP2014}.
...
the networks considered in \tcite{elman1990finding} are such a basic RU, with an extra over-all output layer on-top.
...
First we can see that like in the basic RU, the output and the state are the same value (\Cref{equ:gru-outstate}).
...
\aside[Multiplication, Concatenation, and Addition]{
	...
	$W\cdot \left[\v a; \v b\right] = U\v a + V \v b$  
	if  $W=[U\; V]$
...
\aside[Biasing the Forget Gate]{
	...
	Unlike all the other initialisation in the networks to 0 (or a small random number e.g. \texttt{randn(0.01)}).
