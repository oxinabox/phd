...
\node(L1)[layer, above = of concat1.north east]{$\varphi\left(U\,\dv z_1 + \v b\right)$};
\draw[->]  (concat1.north east) -- node[labe]{$\dv z_1$} (L1);

\node(L2)[layer, above = of L1, label={[label distance=0] 0:\small for each $i$}]{
$\frac{\exp{\i V_{w_i,:} \dv z_2 + \iv k_i}}%
{\sum_{\forall j} \exp{\i V_{j,:} \dv z_2 + \iv k_j}}$
};
\draw[->]  (L1) edge node[labe]{$\dv z_2$} (L2);
