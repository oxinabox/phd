...
\begin{abstract}
	...
	This is an growing area, and is particularly important in languages where polysemous and homonymous words are common (This includes English, but it is even more prevalent in Mandarin for example).
...
\aside[WordNet is not a strong moral baseline]{
	...
	One can imagine the issues that would be caused had ImageNet \parencite{imagenet_cvpr09} had used those counts to determine the synsets for which images would be provided.
	(They didn't: even the 2009 initial release of ImageNet contains hundreds of images of \natlang{wheelchairs}, and \natlang{prosthesis} -- synsets that almost do not occur in the Brown).
...
For input word $w_i$ having senses given by $\mathcal{H}(w_i)$,
...
A softmax classifier can be used to define $P(w_o \mid h_k)$, just like in normal language modelling.
...
Where a training case is a pairing of a word use $w_i$, and context word $w_o$, with $h_l\in \mathcal{H}(w_i)$
\begin{align}
...
During this step the likelihood of the $P(w_o \mid w_i)$ can be optimised to maximise the likelihood of the observations.
This is done via gradient descent on the neural network parameters of the softmax component: $P(w_o \mid h_k)$.
