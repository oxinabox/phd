...
\aside[Stemming]{WordNet is indexed by lexemes in stemmed form. This in essence means with tense and plurality information removed. WordNet comes with a stemmer called morphy to do this.}
\aside[Unstemming]{Given lexeme in stem form (as one can extract from WordNet) and a full POS tag (Such as a Brown, or Penn treebank tag) for a word, it is possible to undo the stemming with a high degree of reliability using relatively simple rules. The POS tag encodes the key features that are removed by stemming: the tense and if it is plural. Patten.en \pcite{de2012pattern}is a python library encoding such rules; though implementing an unstemmer is a task left for the reader. Unstemming can be used to find substitute words using WordNet's features}
