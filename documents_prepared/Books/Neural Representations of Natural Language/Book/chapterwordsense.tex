...
\aside[Semantic Syllepsis]{
	...
	These constructions are also known as zeugma, although zeugma is itself a highly polysemous word, so its usage varies. 
}
...
It could be assumed that as results for these were not reported, they were worse than those found in phase two.
...
These co-location clustering methods require finding a representation for the context, which can then by combined with cosine similarity (or another metric), to give the similarities for clustering.
...
\aside[On clustering]{
	Clustering can be defined as a (mixed integer) optimisation task, of assigning points to clusters so as to satisfy some loss function based around some notion of minimising intra-cluster variance while maximising inter-cluster variance (or similar).
	...
	It should be remembered that there exist many other clustering methods than  k-means and k-means variants.
...
\aside[WordNet and BabelNet]{
	As mentions in the previous sections, WordNet and BabelNet are the predominant lexicons used with word senses. It is not directly relevant to this section, but we have space here to remark upon them.
	WordNet \parencite{tengi1998design} as a very well established tool should have a binding in all modern programming languages.
...
Rather it only only requires that the context words be conditionally independent on on the word in question $\n w_i$.
