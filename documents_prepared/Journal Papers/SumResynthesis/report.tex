...
\begin{table*}		\pgfplotstabletypeset[columns={[index]2,[index]3,[index]4,[index]5,[index]6},
	skip rows between index={0}{4},
	...
	\caption{ The performance of the word selection step, on the Books corpus. Note that the performance of the word selection step, bounds the overall performance shown in \Cref{table:ordering}}. This table shows a subset of the results reported by \textcite{White2015BOWgen}.
...
The overall results the Two Step sentence generation are shown in \Cref{table:overall}.
This can be contrasted with the results of \textcite{White2015BOWgen} for the word selection step, on the same corpus -- which was first half of the Two Step process. The accurasy of the Word Selection (i.e. the BOW generation) bounds the performance of the of the overall process. No matter how good the word ordering method is, it can not recreate perfectly accurate sentences with imperfect words. For this reason the Portion Perfect of word selection is a strict upper bound to the Portion Perfect of the overall. It can be seen that the additional errors made during ordering caused an imperfections in a further 13.4\% of sentences. Less straight-forward is the bounding of overall BLEU score, by word selection Precision.

 The table also includes Oracle results, which only assess the ordering (without assessing the section) The Oracle bypasses the selection step, by providing to the ordering step randomly shuffled reference sentences. It is presented for comparison of the best possible performance using the ordering method -- assuming the word selection step could be done perfectly.
