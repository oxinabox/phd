...
\setlength{\intextsep}{8pt plus 4pt minus 4pt}
...
\textcite{White2015BOWgen} approaches the BOW generation problem, by as a the vector selection problem -- selecting the vectors that sum up closest to a given vector. This is related to the knapsack and subset sum problems. They formally define the vector selection problem as:
\[
...
Resynthesis degraded as sentence length increased. \textcite{White2015BOWgen} showed that the word selection degrades with sentence length, and improves with higher dimensional embeddings. Due to their findings, we only evaluated 300 dimensional embeddings. The technique was only evaluated on sentences with up to 18 words (inclusive), due to computational time limitations. On these it performed quite well achieving perfect recreation in 62.2\% of cases. To the author's knowledge our proposed method is the first to report results for the attempted exact recreation of a substantive corpus. Both accuracy and running time worsens as sentence length increases. With that said, short sentences are sufficient for many practical uses.
