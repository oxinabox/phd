...
\begin{frame}[label=twostep]{We can break the problem down into two subproblems.}
	...
	\end{tikzpicture}
	\vfill
	\only<1>{\alert{Vector Selection}: Select which word vectors go into the sum}
	\only<2>{\alert{Word Ordering}: Find them most likely order of words}
...
\newcommand{\vectorselectionproblemdefn}{Find the \alert{inclusion vector} $\c=[c_1,c_2,...c_n]\in\N_0^n$ to \alert{minimise}
	 $\displaystyle d(\s,\:\: \sum_{\x_j\in\V}\:\x_{j}\,c_{j})$ 
}
...
\begin{frame}{Run until convergence}
	...
	\begin{tikzpicture}[
		...
		\draw[->] (input) -- (addition);
		\path[->] (addition) edge [bend left=90,looseness=2] (addition);
...
\begin{frame}{Results: The more dimensions used in the word performance, the better the recovery.}
	\begin{table}
...
\begin{frame}{}
	...
	\only<2>{\frametitle{Result: The larger the vocabulary, the worse recovery}}
...
\begin{frame}
	\frametitle<1>{Results: Word Selection}
	\frametitle<2>{Results: Ordering with Oracle BOW}
	\frametitle<3>{Results: Our complete system}
