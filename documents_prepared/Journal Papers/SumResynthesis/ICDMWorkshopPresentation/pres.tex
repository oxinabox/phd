...
\begin{frame}{Related problems}
	...
	\end{itemize}
	However, sentence generation is district as we go from a vector representation that encodes meaning, rather than from one that encodes memory.
\end{frame}
...
\newcommand{\vectorselectionproblemdefn}{Find the inclusion vector $\c=[c_1,c_2,...c_n]\in\N_0^n$ to minimise
	that for $\displaystyle d(\s,\:\: \sum_{\x_j\in\V}\:\x_{j}\,c_{j})$ 
}
