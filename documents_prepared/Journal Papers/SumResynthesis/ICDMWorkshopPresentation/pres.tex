...
\date{}
\logo{\includegraphics[scale=0.12]{uwa}\hspace{1.5cm}}
...
\newcommand{\vectorselectionproblemdefn}{Find the \alert{inclusion vector} $\c=[c_1,c_2,...c_n]\in\N_0^n$ to \alert{minimise}
	 $\displaystyle d(\st,\:\: \sum_{\x_j\in\V}\:c_{j}\,\x_{j})$ 
}
...
\newcommand{\collenone}{3.1cm}
\newcommand{\collentwo}{5.7cm}
\newcommand{\collenthree}{0.5cm}

\begin{frame}<2>[label=lstmeg]{LSTM-VAE Examples}
	\small
	\hspace*{-8pt}\makebox[\linewidth][c]{
		\begin{tabular}{ p{\collenone} p{\collentwo} C{\collenthree} C{\collenthree} }
			\only<1>{
				\textbf{ \reftitle}  & we looked out at the setting sun . & \textbf{Sel.} & \textbf{Ord.} \\
				...
				\textbf{ \reftitle}  & i went to the kitchen . & \textbf{Sel.} & \textbf{Ord.} \\
				...
				\textbf{ \reftitle}  & how are you doing ? & \textbf{Sel.} & \textbf{Ord.} \\
...
\begin{frame}<1,3>[label=dtraweg]{DT-RAE Examples}
		...
		\hspace*{-6pt}\makebox[\linewidth][c]{
			...
			\only<1>{			
			\textbf{ \reftitle}  & name this 1922 novel about leopold bloom written by james joyce . & \textbf{Sel.} & \textbf{Ord.} \\
			...
			\only<2>{
			\textbf{ \reftitle}  & ralph waldo emerson dismissed this poet as the jingle man and james russell lowell called him three-fifths genius and two-fifths sheer fudge . & \textbf{Sel.} & \textbf{Ord.} \\
			...
			\only<3>{
			\textbf{ \reftitle}  & this is the basis of a comedy of manners first performed in 1892 . & \textbf{Sel.} & \textbf{Ord.} \\
			...
			\only<4>{
			\textbf{ \reftitle}  & in a third novel a sailor abandons the patna and meets marlow who in another novel meets kurtz in the congo . & \textbf{Sel.} & \textbf{Ord.} \\
			...
			\only<5>{
			\textbf{ \reftitle}  & thus she leaves her husband and child for aleksei vronsky but all ends sadly when she leaps in front of a train . & \textbf{Sel.} & \textbf{Ord.} \\
...
\begin{frame}<2>[label=ambeg]{Ambiguous Examples}
	\small
	\hspace*{-8pt}\makebox[\linewidth][c]{
		\begin{tabular}{ p{\collenone} p{\collentwo} C{\collenthree} C{\collenthree} }
			\only<1>{			
			\textbf{ \reftitle}  & it was the worst of times , it was the best of times . & \textbf{Sel.} & \textbf{Ord.} \\
			...
			\textbf{ \reftitle}  & please give me directions from Paris to London . & \textbf{Sel.} & \textbf{Ord.} \\
...
\begin{frame} %Hack: I can't get \only working within a plot so just repeat it 3 times
	...
	\begin{axis}[fullresplot]
	only<1->{\addplot table [y=books_0_01_glove300_perfect_mean, x index=0, skip coords between index={18}{1000}]{\sellenscores};}
	only<2->{\addplot table [y=Perfect Sentences,x=ground_length]{\ordlenscoresoracle};}
	...
	only<1->{\addplot table [y=books_0_01_glove300_perfect_mean, x index=0, skip coords between index={18}{1000}]{\sellenscores};}
	only<2->{\addplot table [y=Perfect Sentences,x=ground_length]{\ordlenscoresoracle};}
	only<3->{\addplot table [y=Perfect Sentences,x=ground_length]{\ordlenscores};}
