...
\newcommand{\drawgroupinner}{
	...
	\draw (\gx,\gy-3*\ystep cm) ellipse (0.7cm and \ystep*5 cm);
}
...
\newcommand{\drawgrouptop}[3]{%
	...
	\drawgroupinner{}	
	\node at (\gx,\gy+2.8*\ystep cm) {$S($\Words(\i)$)$};
...
\newcommand{\drawgroupbottom}[3]{%
	...
	\renewcommand{\gy}{#3 cm}
	\drawgroupinner{}	
	\node at (\gx,\gy-8.5*\ystep cm) {$S($\Words(\i)$)$};	
}
...
\drawgroupbottom{3}{5}{-2.5}
\drawgrouptop{4}{6.5}{2.5}
\drawgrouptop{5}{9}{0}
