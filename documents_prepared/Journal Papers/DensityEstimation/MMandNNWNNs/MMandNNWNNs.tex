...
\begin{abstract}
In this work we prove that boundedness and monotonicity are necessary and sufficient conditions for a Nonnegatively Weighted Neural Network (NNWNN) to converge to its target function.
...
\begin{thm} \label{thm:upper}
	For any NNWNN, represented as a function $N:\R \to \R$ satisfies the following properties:
	
	\begin{enumerate}
		\item $N$ is continuous \pdfcomment{idk if I need this statement it is true, but idk if it is useful}
		\item $N$ is monotonically increasing
		\item $|N|<M$ for some $M\in \R$, i.e. $N$ is bounded above and below
	\end{enumerate}
\end{thm}
\begin{cor}\label{cor:noapproxapproximate}
	\pdfcomment{Chris thinks this probably does not need to be stated as it is obvious}
...
\begin{thm} \label{thm:nnwnnuat}
	For such a function $F:\R \to \R$ that is monontically increasing, continuous and bounded,
	for all points  $x$in its domain of $F$
	...
	That is to say:Nonnegativly weighted neural networks (NNWNN) are able to approximate any monotonically increasing functions bounded above and below.
...
\section{NNWN Facts}
\pdfcomment{I need to retitle this seciton and move it around, but I just need to exstablish things things first}
...
\begin{defn}{Single-Input Single-Output NNWNN}
	...
	\begin{equation}
	N(x;W,V,b,c) = \sum_{j=1}^{j=n_h} V_{1,j} \varphi (b_j+W_{j,1}x_1) + c_1 \label{equ:singleNNWNN}
	\end{equation}
\end{defn}

It is implied from \Cref{equ:singleNNWNN} the network's derivative with respect to its input $x$ is given by
...
{\scriptsize
	\begin{align}
	J_N&=\left(V\odot W^{T}\right)g(Wx+b)\\
...
\begin{lem}
	...
	\pdfcomment{I have I think a proof that this works for noncontinous summability kernals too, at least for bounded functions}
	
	\pdfcomment{Insert a citation here}
\end{lem}


\begin{lem}\label{lem:squashderivissum}
	The derivative of any absolutely continuous almost everywhere squashing function gives rise to a summability kernel.
    ...
    \pdfcomment{Maybe I should use varphi prime instread of g in this section, it might be clearer}
    
	\begin{equation}
	g_n(z) \coloneqq \frac{k_n g(k_n z)}{ c_{max} - c_{min}} 
	\end{equation}
	 where $c_{min}=\lim_{z \to -\infty} \varphi(z)$ and $c_{max}=\lim_{z \to \infty} \varphi(z)$,
	 and $k_n$ is the $n$th element of some sequence of real numbers such that $\lim_{n\to \infty} k_n \to \infty$.
	This sequence is always a summability kernel, for any squashing function
	that is .
	
	\pdfcomment{Need to put in the generalisation to a.e continuous, eg RELU6, but I know this works anyway, see notes.}
...
For the sigmoid activation function it is a bell-curve, as shown in \Cref{fig:sigmoidbell}, for RELU6 it is a uniform distribution.
These are valid PDFs are they are non-negative, and have unit area (as was shown in \Cref{lem:squashderivissum}).
From this, and the $V$ is non-negative, we can note that \Cref{equ:pdf2} defines a mixture model in the standard form, with $n_h$ components.
...
\pdfcomment{TODO Replace this with a nicer plot}
\end{figure}
