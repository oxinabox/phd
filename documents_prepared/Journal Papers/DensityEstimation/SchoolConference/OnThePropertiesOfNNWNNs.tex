...
\begin{frame}{Why NNWNNs?}
	...
	\begin{itemize}
		...
		\note{Though for the clase of binary classifications then the top layer can be interpretted as slices in a way that Chris will probably talk about.}
		\item This decrease the back-box natural of NNs
		\item This is similar to Nonnegative Matrix Factorisation (NMF)
	\end{itemize}

	\citehere{2014understandblenetworksnonnegweights}
\end{frame}
...
\begin{frame}{But look, it is actually a Almost GMM}
	\begin{block}{Look a but closer}
		\begin{align*}
		f(x) &\approx \dfrac{}{\sum_{k=1}^{k=n_h} V_{1,k}} \sum_{j=1}^{j=n_h} V_{1,j}\, \bigg(W_{j,1} \vp (b_j+W_{j,1}x_1) \bigg)
		\end{align*}
	\end{block}
