...
From the consideration of all results (\Cref{tbl:pointextrapo}, \Cref{tbl:distextrapo}, \Cref{tbl:pointfull}, \Cref{tbl:distfull}, \Cref{tbl:distord}, \Cref{tbl:pointord})
...
This is seen consistently in all the distribution estimation tasks  (\Cref{tbl:distfull}, \Cref{tbl:distord}, \Cref{tbl:distextrapo}).
...
This is because without smoothing it results in estimating the probability based on bins unfilled by any observation (Which is zero due to the cap on the minimum value. See \Cref{sec:direct-histogram}).
This is particularly notable in the case of the direct, (unsmoothed) non-extrapolating result reported in \Cref{tbl:distextrapo}. As these were some of the rarest terms in the training set, they were thus more likely to not coincide with any terms from the testing set.
