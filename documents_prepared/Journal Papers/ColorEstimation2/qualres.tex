...
\newcommand{\pointfigs}[3]{
	...
	\foreach[count=\mdlii from 0] \mdlfile/\mdlname in #1 {%
		...
		\node[above = -0.7 of fig\mdlfile] {\sffamily \mdlname};%
	}%
	\end{tikzpicture}%
}%
...
This is reflected in in \Cref{fig:distout1} and \Cref{fig:pointout},
...
\subsection{On smoothness in learned distribution estimates}
In \Cref{fig:distout1} can be seen that the term-based distribution estimation models are much smoother than corresponding histograms taken from the the training data.
...
This aligned with the results presented in \Cref{sec:smoothed-results}.
