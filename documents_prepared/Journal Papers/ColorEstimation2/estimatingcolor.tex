...
To establish an rough upper-limit on the modeling results we evaluate a direct method which bypasses
...
This operational uppper bounds bypass the compositional language understanding part of the process.
...
A term based model could out perform them, however this would require learning how the terms in the color name combine in a way that exceeds the information directly present in the training data per class.
...
we use kernel-density estimation in a formulation for non-parametric estimation \parencite{silverman1986density}.
