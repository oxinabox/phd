...
\newcommand{\empmodel}{non-compositional baseline} % Note this is not used everywhere
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%opening
\runningtitle{Learning of Colors from Color Names: Distribution and Point Estimation}
\runningauthor{Lyndon White et. al.}
%Document Head
...
\affil{Invenia Labs, Cambridge, UK\\ \& The University of Western Australia}
...
\affil{The University of Western Australia
	%\thanks{The University of Western Australia, 35 Stirling Highway, Crawley, Western Australia.}
}
...
We argue that the latter has a particular value as there is no clear agreement between people as to what a particular color describes -- different people have a different idea of what it means to be ``very dark orange'', for example.
Surprisingly, despite it's simplicity, the sum of word embeddings generally performs the best on almost all evaluations.
\end{abstract}
...
This can be seen in the histograms of the training data shown in \Cref{fig:distout1} in \Cref{sec:qualitative-results}.
...
It is followed closely by the CNN; with the RNNs both performing significantly worse (see \Cref{sec:results}).
...
%
%To the best of out knowledge, this is the first of such investigation on term-wise mapping color names to values that can generate visual representations, which bring the future of natural language controlled computer graphics generation and executable semantic parsing one step closer to reality.  
...
Full details on this dataset can be found in \Cref{sec:full-task}.
...
\textcite{acl2018WinnLighter} demonstrates a neural network for producing directional vectors in a color space indicating how comparatives such as \natlang{lighter} and \natlang{darker} change a color.
...
For example mapping from \natlang{green} to a \natlang{darker green} is: $((164,227,77), \text{darker}) \mapsto (141, 190, 61)$ for a point estimate.
...
\section{Method}
\begin{figure}
	\resizebox{\textwidth}{!}{\begin{tikzpicture}[defaultfig]
		...
		\node (input) [matrix, right = of prep] {
			\node{Input Modules}; \\
			\node(rnn1)[draw]{GRU RNN}; \\
			\node(rnn2)[draw]{LSTM RNN}; \\
			\node(sowe)[draw]{SOWE};\\
			\node(cnn)[draw]{CNN}; \\
		};
		\refnodeBR{sec:sowemod}{sowe};
		\refnodeBR{sec:cnnmod}{cnn};
		\refnodeAR[]{sec:rnnmod}{rnn1};
		\draw[red] (rnn1ref.south) to (rnn2.50);
		...
		\node(dist)[right=of softmax]{Probability\\Distribution\\Estimate};
		...
		\end{tikzpicture}}
	\caption{The overall architecture of our system.\label{fig:arch}}
\end{figure}
...
\end{itemize}
The final example of \natlang{blurple} $\mapsto$ [\natlang{blue-purple}] is a special-case.
...
One set of training histograms is produced per color description present in the dataset -- that is to say a training histogram is created for \natlang{brownish green}, \natlang{greenish brown}, \natlang{green} etc.
...
then observation at 2, then 50\% of probability mass for this observation would be allocated to the bin with midpoint 1, and 50% would be allocated to the bin with midpoint 2.
...
For each of the models investigated we define an input module.
...
%
%{For reference the parameter counts, on top of embedding layer
%	LSTM has 5x300x301 (4 internal + final ReLU)
%	GRU has 4x300x301 (3 internal + final ReLU)
%	CNN has 2x300x301 (Convolution including ReLU final ReLU)
%	SOWE has 300x301 (final ReLU only)
%
%Maybe this summary should be included somewhere.
%}


\subsubsection{Recurrent Neural Networks (GRU and LSTM  RNNs)}\label{sec:rnnmod}
\begin{figure}
	...
	\node (GRU1)[layer, below = of hiddenoutput]{GRU/\\LSTM};
	
	\foreach[count=\i from 1] \j in {2,...,5}
	{
		\node (GRU\j)[layer, left = of GRU\i]{GRU/\\LSTM};
		\draw[->] (GRU\j) to (GRU\i);
	}
	...
	\caption{The LSTM/GRU Input module
...
The general structure of both input modules is  shown in \Cref{fig:rnnmod}.
...
\end{align}
The mean of this loss is taken over all observations in each mini-batch during training.
...
As there is plenty of training data for most color names (see \Cref{sec:fulltaskcorpusstats}) this is a very effective approach.
%
...
In theory the term-based neural models shoukd out-perform the \empmodel{}, if they learn a very good compositional understanding of the language.
...
\emph{It is this capacity of learning how the terms combine that allow for the models to predict the outputs for combinations of terms that never occur in the training data (\Cref{sec:extrapodata}).}
...
Conversely, when there is no training data for the whole phrase (as considered in \Cref{sec:extrapodata}) non-compositional models can not function at all.
...
\begin{figure}
	\distfigs{{SOWE/SOWE, CNN/CNN, GRU/GRU, LSTM/LSTM}}{\oovcolors}	
...
\begin{figure}
	\pointfigs{{SOWE/SOWE, CNN/CNN, GRU/GRU, LSTM/LSTM}}{oovcolors}{5}
