...
\affil{Invenia Labs, Cambridge, United Kingdom}
...
\affil{The University of Western Australia\thanks{The University of Western Australia, %
...
\todo{For reference the parameter counts, on top of embedding layer
...
The general structure of both input modules is  shown in \Cref{fig:rnnmod} is similar to \textcite{2016arXiv160603821M}, or indeed to most other word sequence learning models.
...
\todo{ What do with following sentence?
We make use of a GRU
which we found to marginally out-perform a vanilla RNN (that is to say one without any gating) in preliminary testing.
The small improvement is unsurprising, as the color names have at most 5 terms,
so longer short term memory (from GRU or LSTM) is not as useful required.}
...
As there is plenty of training data for most color names (see \Cref{sec:corpusstats}) this is a very effective approach.
%
