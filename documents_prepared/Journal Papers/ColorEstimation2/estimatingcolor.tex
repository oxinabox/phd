...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%opening
\title{From color names to colors \\(distribution and point estimation)}
\author{Lyndon White, %
	Roberto Togneri, %
	Wei Liu, %
	 Mohammed Bennamoun%
...
For example, in a natural language enabled human-machine interface, when asked to select the \texttt{dark bluish green} object, each object can be ranked based on how likely its color is according to the distribution.
...
Thus the \emph{color intended} cannot be uniquely inferred from a color name.
Without further context, it does nevertheless remain possible to estimate likelihoods of which colors are  intended based on the population's use of the words.



Proper understanding of color names requires considering \emph{the color intended} as a random variable.
...
A single point estimate, does not capture the diverse nature of the color names adequately.

Moreover, it is impossible to find the single best point estimation method.
...
The generation of color from text has not received a significant amount of attention in prior work.
The only similar work we are aware of is \textcite{DBLP:journals/corr/KawakamiDRS16};
which only considers point estimation,
and which uses a dataset that does not contain enough observations to learn (or assess) probability distributions based on the populations usage of the color names.
...
Conversely, there has been several works on the reverse problem \parencite{mcmahan2015bayesian,meomcmahanstone:color,2016arXiv160603821M}: the generation of a textual name for a color from color space point.
...
In this research we use the same dataset \parencite{Munroe2010XKCDdataset}
...
To qualify our estimate of the distribution, we discretize the space to produce a histogram.
...
We investigated using a Kernel Density Estimation (KDE) based method for smoothing the training data, and further we conclude that the neural network based models are able to learn this smoothness with or without the smoothing of the training data.% (Results for this are in \Cref{sec:smoothed-results}).
Such smoothing is however essential when directly estimating from the training data.



\subsection{Contributions}
Problem statement: given a set of \datapairs{} pairs, we need to learn a mapping from any color-name, seen or unseen to a color-value or a distribution in HSV space.
...
These include : \pdfcomment{I'm by do use dot points or something}
...
To the best of knowledge, this is the first of such investigation in mapping color names to values that can generate visual representation, which bring the future of natural language controlled computer graphics generation one step closer to reality.  
