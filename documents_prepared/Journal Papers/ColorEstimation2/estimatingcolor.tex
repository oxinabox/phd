...
\affil{Invenia Labs, Cambridge, UK}
...
This can be seen in the histograms of the training data shown in \Cref{sec:qualitative-results}.
...
\textcite{mcmahan2015bayesian} and \textcite{meomcmahanstone:color} present a Bayesian method for colors estimation and color naming.
...
\textcite{acl2018WinnLighter} demonstrates a neural network for producing directional vectors in color space indicating how comparatives such as \natlang{lighter} and \natlang{darker} change a color.
This effectively defines a ray (half-line) in color space along which possible colors described lie.
Their networks takes as it's inputs a word embedding for a comparative adjective, and a point in RGB color-space.
It outputs a directional vector in RGB space along which occurs the possible modified version of the input color point according to the given adjective.
...
as it does not have a tokenized color name given to it: only the whole name.
...
%
These models can exploit the training observations without the issue of determining how to compose the tokens.
This is a useful baseline approach, as our neural models (SOWE, CNN, GRU and LSTM)
each differs in how they compose the tokens, and it is on that contrast this study focuses.

In theory the term-based neural models can out-perform the \empmodel{}, if they learn a very good compositional understanding of the language.
...
This is particularly true in the case where there is large amounts of training data for the whole phrase.
...
\begin{itemize}
	...
	\item 20,460 observations were removed from the training set . Thus the restricted training set has 13.43\% fewer observations than the full training set.
...
\begin{itemize}
	\item There are 76 unique color names with 2 possible orders for their terms. This makes up 9.17\% of the unique color names.
	...
	\item There are 16 terms used in these ambiguous ordered color names. Namely:
	\natlang{apple}, \natlang{blue}, \natlang{bright}, \natlang{brown}, \natlang{green}, \natlang{grey}, \natlang{ish}, \natlang{light}, \natlang{orange}, \natlang{pink}, \natlang{purple}, \natlang{red}, \natlang{violet}, \natlang{y}, \natlang{yellow} and \natlang{-}.
\end{itemize}
