...
To further evaluated the models, and there capacity in this natural language understanding task we consider both the creation of point estimates of color as well as distribution estimates of color.
...
We find consistent results across the point estimation and distribution estimation tasks.

We investigate 3 different slices of the data, to assess if different properties hold.
We define the full task to assess overall performance, full training and testset.
We define the ordered task to assess performance when word order may matter, using a subset of the testset with only color descriptions that occur with the same words in different orders.
We define the unseen combinations task to assess the ability of the models to make predictions for color names made up of words that have never been seen in that combination during training; restricting the training set to not contain certain combinations and the test-set to only contain those combinations.
We find our results to be consistent across all subtasks.
This is particularly interesting as the unordered sum of word embeddings performs best on the ordered evaluation task.
\end{abstract}
...
However we do consider the task, as it remains an interesting challenge for assessing the systems under evaluation.
...
It is however essential when directly estimating from the training data, without a machine learning step that can learn this smoothness relation.
...
\begin{itemize}
	...
	\item for color names which never occur in the training data exactly, but for which all terms occur in the training data (unseen combinations task)
...
\subsection{Perplexity in Color-Space (Evaluation Metric for Distributions)}
...
\subsection{Output Modules}\label{sec:outputmod}
...
This is done using the softmax output layers, as shown in \Cref{distoutmod} the three output layers.
...
We also investigated applying the KDE to the training data,  before training our neural network based distribution models.
...
and directly uses non-parametric probability estimation methods to focus solely on modelling the distributions.
...
Taking the mean of all the observations give the theoretically optimal solution to minimizing the mean squared error on the training data set.
...
\pdfcomment{Merges these parragraphs}
We note that point estimation using more training items of lower information content each.
...
This means that the distribution estimation networks have a smaller number of training items (one per full color name), but that each item is much richer in information, being the entire distribution.
Where as the point estimation methods have more training items (one per raw observation) but that each only contains a single a point.
The total information content of the training data for both is equal.
However, we suspect that many low-information items processed via random minibatches will allow for more effective learning.
